...
var openDiseaseInfoCommonQuery<%=property%> = function (){
	    ...
	    var url="pages.sysorg.org.PublicWindow&aa="+Math.random()+"&property=<%=property %>&codetype=<%=codetype %>&codevalue="+codevalue+"&closewin="+winId+"&codename="+codename+"&codevalueparameter="+codevalueparameter+"&subWinId="+winId;
	   	//alert(url);
	   	var label='<%=label2!=null?label2:label %>';
...
<odin:select2 property="<%=property %>" required="<%=required%>" value="<%=defaultValue%>" label="<%=label%>"
  ondblclick="<%=ondblclick %>"  codeType="<%=codetypeSelect %>" onchange="<%=onchange %>" codename="<%=codename %>"
