...
<textArea name="<%=property%>" style="display:none;"
	...
	onfocus="<%=onfocus==null?"":onfocus%>" onblur="<%=onblur==null?"":onblur%>" ></textArea>
...
//显示div 隐藏输入控件
function <%=property%>onblur(a,b,c,isnatural,n){
	...
	document.getElementById("div_<%=property%>").innerText=document.getElementById("<%=property%>").value;
	ajust<%=property%>();
	...
	if(onblurFAjust&&isnatural!==false){
		onblurFAjust('<%=property%>');
	}
...
function ajust<%=property%>(){
	...
	var div_t = document.getElementById('div_<%=property%>').offsetTop-2;
	if(Ext.isIE){//alert(wrapdiv_h+','+div_t);
		if((wrapdiv_h+div_t)<=0){
			document.getElementById('div_<%=property%>').style.top=-wrapdiv_h+2;
		}else{
			document.getElementById('div_<%=property%>').style.top='-50%';
		}
	}else{
