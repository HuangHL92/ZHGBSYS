	...
	function loadRoleGridData(){
		...
		var params = {};
		var sql = "@_OzvAUCeqM5WIpgRNt5cbChEqTaJAzPGLgVdMaol37ShZrcLDTjAYVPPVlVFo/GwsDo8NJ7D5l38XC6EGUGYooDlAt52qurZ3UItgQcLQmqs7dxw/nGb5z71qq0r1D5IcAABDPCJSVJk=_@";
//		if(sysId == ""){
//			sql += "@_K6ys7lmbMBAromMRdtkOag==_@";
//		}else{
//			sql += "@_K6ys7lmbMBAromMRdtkOag==_@ or @_jVsDz+UHfJU=_@"+sysId+"@_fqOngwdfJf8=_@";
//		}
		if(roleName!=""){
			sql += "@_3QplZ2y8dDnUnYqYDym4Glq8zGs5kdeN_@"+roleName+"@_1BSPFBy2FCc=_@";
		}
		if(roleDesc!=""){
			sql += "@_3QplZ2y8dDm22FrNFBmp2Vq8zGs5kdeN_@"+roleDesc+"@_1BSPFBy2FCc=_@";
		}

		sql += "@_0lLHxReFyQFn7M15OYyfEa1qkPBx3KCn_@";
		params.querySQL = sql;
		params.sqlType = "SQL";
	...
	function doAfterEdit(e){
				...
				if (record.get('roleid') != "") { //说明是编辑
					...
					//没修改
						...
						if (rn != "") {
							//唯一性校验
							var params = {};
							var sql = "@_OzvAUCeqM5W1Wpyf+tVuWGS+whADGLtnM0WHLAZviwHkWSbP+qE/e7I/gBQ/qzvYnaWiPvSjbag=_@" + rn + "@_fqOngwdfJf8=_@";
							params.querySQL = sql;
							params.sqlType = "SQL";
		...
		if(isSubmit){
			...
			fm.qxlx.value = record.get('qxlx');
			fm.sysid.value = record.get('sysid');
			//odin.alert(fm.qxlx.value);
	...
	function renderStatus(value, params, record, rowIndex, colIndex, ds){
		if(value=='1'){
			return "<img src='../commform/img/wrong.gif' title='角色名称有重复，修改失败！'>";
		}else if(value=='2'){
			return "<img src='../commform/img/right.gif' title='修改角色成功！'>";
		}else if(value=='3'){
			return "<img src='../commform/img/wrong.gif' title='角色名称不允许为空！'>";
		}else if(value=='4'){
			return "<img src='../commform/img/right.gif' title='新增角色成功！'>";
		}else{
			return '';
		}
	}
	...
	function renderOpFun(value, params, record, rowIndex, colIndex, ds){
		return "<img src='../commform/images/icon/icon_photodesk.gif'  onclick='openFunWin(\""+value+"\",\""+record.get("sysid")+"\",\""+record.get("userid")+"\")' title='给此角色授权'>";
	}
	/**
	 * 打开授权窗口
	 * @param {Object} roleid
	 */
	function openFunWin(roleid,sysid,id){
		//打开授权窗口
		userid=id;
		cueRoleId = roleid;
		sysId = sysid;
	...
	function addRowForNewUser(){
		odin.addGridRowData('rolegrid',{roleid:'',roledesc:'',oldrolename:'',sysid:'',rolename:'',qxlx:''});   
	}
	/**************************************/
