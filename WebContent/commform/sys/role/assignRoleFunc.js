...
function onLoad(){
    ...
    tree = new dhtmlXTreeObject("treeBox", "100%", "360", 0);
	tree.setImagePath(contextPath+"/commform/basejs/tree/imgs/csh_bluebooks/");
	...
	if (isOnceLoad == "1") {
		...
		if(parent.userid && parent.userid !=""){
			...
			parent.cueRoleId +
			"&sysid=" +
			parent.sysId+"&userid=" + parent.userid);
		}else{
			tree.loadXML(contextPath + "/sys/roleAction.do?method=getAllFuncXmlInfo&roleid=" +
			parent.cueRoleId +
			"&sysid=" +
	...
	timeObj = window.setInterval("testIsLoadComplete()",500);
}
function testIsLoadComplete(){
	//if(tree.getItemText('S000000')!=null){
...
function submit(){
	...
	fm.userid.value="";
	if(parent.userid && parent.userid !='undefined'){
		fm.userid.value=parent.userid;
	}
