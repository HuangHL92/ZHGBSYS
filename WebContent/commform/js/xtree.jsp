<%@page contentType="text/javascript; charset=GBK"language="java"%>var webFXTreeConfig={rootIcon:'<%=request.getContextPath()%>/images/xp/folder.png',openRootIcon:'<%=request.getContextPath()%>/images/xp/openfolder.png',folderIcon:'<%=request.getContextPath()%>/images/xp/folder.png',openFolderIcon:'<%=request.getContextPath()%>/images/xp/openfolder.png',fileIcon:'<%=request.getContextPath()%>/images/xp/file.png',iIcon:'<%=request.getContextPath()%>/images/xp/I.png',lIcon:'<%=request.getContextPath()%>/images/xp/L.png',lMinusIcon:'<%=request.getContextPath()%>/images/xp/Lminus.png',lPlusIcon:'<%=request.getContextPath()%>/images/xp/Lplus.png',tIcon:'<%=request.getContextPath()%>/images/xp/T.png',tMinusIcon:'<%=request.getContextPath()%>/images/xp/Tminus.png',tPlusIcon:'<%=request.getContextPath()%>/images/xp/Tplus.png',blankIcon:'<%=request.getContextPath()%>/images/xp/blank.png',defaultText:'ÕýÔÚ¼ÓÔØ...',defaultAction:'javascript:void(0)',defaultBehavior:'classic',usePersistence:false};var webFXTreeHandler={idCounter:0,idPrefix:"webfx-tree-object-",all:{},behavior:null,selected:null,onSelect:null,getId:function(){return this.idPrefix+this.idCounter++;},toggle:function(oItem){this.all[oItem.id.replace('-plus','')].toggle();},select:function(oItem){this.all[oItem.id.replace('-icon','')].select();},focus:function(oItem){this.all[oItem.id.replace('-anchor','')].focus();},blur:function(oItem){this.all[oItem.id.replace('-anchor','')].blur();},keydown:function(oItem,e){return this.all[oItem.id].keydown(e.keyCode);},cookies:new WebFXCookie(),insertHTMLBeforeEnd:function(oElement,sHTML){if(oElement.insertAdjacentHTML!=null){oElement.insertAdjacentHTML("BeforeEnd",sHTML)
return;}
...
this.rendered=true;return str+sb.join("")+"</div>";};function WebFXTreeItem(sText,sAction,isMainMenu,eParent,sIcon,sOpenIcon){this.base=WebFXTreeAbstractNode;this.base(sText,sAction);this.isMainMenu=isMainMenu;if(webFXTreeConfig.usePersistence){this.open=(webFXTreeHandler.cookies.getCookie(this.id.substr(18,this.id.length-18))=='1')?true:false;}else{this.open=false;}
