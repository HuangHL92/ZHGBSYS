...
<%
	//FR独立工程测试用地址： http://127.0.0.1:8090/insiis6/pages/commRep2Action.do?method=test&query=true
	String pagePaneUrl = "";
	String FR_ADDR=HBUtil.getValueFromTab("aaa005","aa01","aaa001='FR_ADDR'");
	if(FR_ADDR!=null){
		if(!FR_ADDR.endsWith("/fr")){
			if(FR_ADDR.endsWith("/")){
				FR_ADDR += "fr";
			}else{
				FR_ADDR += "/fr";
		...
		FR_ADDR = request.getContextPath();
	}
%>
<html>
<head>
...
<link rel="stylesheet" type="text/css"
	href="<%=FR_ADDR%>/ReportServer?op=resource&resource=/com/fr/web/core/page.css"></link>

<link rel="stylesheet" type="text/css"
	href="<%=FR_ADDR%>/ReportServer?op=resource&resource=/com/fr/web/load.css"></link>

<script type="text/javascript"
	src="<%=FR_ADDR%>/ReportServer?op=resource&resource=/com/fr/web/load.js"></script>
	...
	<odin:form action="/pages/comm/commAction.do?method=doAction"
		...
		<%
			String method = request.getParameter("method");
					...
					String param1 = "";//SysfunctionManager.getModuleSysfunction().getParam1(); //jinwei 2013.6.3
					...
					pagePaneUrl = FR_ADDR + "/ReportServer?reportlet=" + pagePaneUrl;
...
<script type="text/javascript">
	...
	function pagePaneLoad(){ //加载
		...
		src = src + "&fr_filename=测试";//src = src + "&fr_filename="+((href.indexOf(".do?")!=-1?MDParam.title:"") + Ext.util.Format.date(new Date(), "Ymd")); jinwei 2013.6.3
