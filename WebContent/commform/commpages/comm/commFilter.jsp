...
<odin:toolBar property="filterToolBar">
	...
	<odin:buttonForToolBar text="增加" handler="doAddRow"
		icon="/commform/basejs/ext/resources/images/default/dd/drop-add.gif"
		cls="x-btn-text-icon"></odin:buttonForToolBar>
	<odin:buttonForToolBar isLast="true" text="确定" handler="doCommFilter"
		icon="/commform/basejs/ext/resources/images/default/dd/drop-yes.gif"
		cls="x-btn-text-icon"></odin:buttonForToolBar>
</odin:toolBar>
...
<script type="text/javascript">
	...
	function commFilter(){
		...
		if(store.getCount()>0){
			for(var i=0;i<store.getCount();i++){
				...
				if(record.get("col")!="" && record.get("relation")!=""){
					if(filterString!=""){
						...
						if(filterType=="and"){
							filterType = "@_l3D6r+QaJro=_@";
						}else if(filterType=="or"){
							filterType = "@_onL5r8EEuQs=_@";
						}
			...
			if(querySQL.indexOf("/*flt*/")>=0){ //要加入过滤语句的地方有注释
				querySQL = querySQL.replace("/*flt*/", filterString);
			}else if(querySQL.indexOf("/*flt-start*/")>=0){//原已经过滤必须在过滤的地方加/*flt-start*/和/*flt-end*/包围的注释
				var filterStart = querySQL.indexOf("/*flt-start*/");
				var filterEnd = querySQL.indexOf("/*flt-end*/")+11;
				var repStr = querySQL.substring(filterStart, filterEnd);
				querySQL = querySQL.replace(repStr, filterString);
			}else{ //未加注释
				if(sqlType.toUpperCase() == "HQL"){
					querySQL = querySQL + filterString;
				}else{
					querySQL = "@_76tJBvkT+4WvW8Ix/e6qDg==_@" + querySQL + "@_7y/+a0omhqA=_@"+filterString;
				}
			}
	...
	function renderComboValue(a,b,c,d,e,f){v=doGridvalueColSelect(a,b,c,d,e,f);renderEdit(v,b,c,d,e,f);return renderAlt(v,b,c,d,e,f)}
	function renderTextValue(v,b,c,d,e,f){renderEdit(v,b,c,d,e,f);return renderAlt(v,b,c,d,e,f)}
	function renderNumberValue(v,b,c,d,e,f){renderEdit(v,b,c,d,e,f);return renderAlt(v,b,c,d,e,f)}
	function renderDateValue(a,b,c,d,e,f){v=renderDate(a,b,c,d,e,f);renderEdit(v,b,c,d,e,f);return renderAlt(v,b,c,d,e,f)}
	function renderDateTimeValue(a,b,c,d,e,f){v=renderDateTime(a,b,c,d,e,f);renderEdit(v,b,c,d,e,f);return renderAlt(v,b,c,d,e,f)}
	function setFilterGridData(gridId) {
		...
		if (document.all(inputName).value != null && document.all(inputName).value != "") {
			...
			if (jsonStr != null && jsonStr != "") {
				...
				e.field = 'value';
				e.originalValue = '';
				e.column = getGridColumnIndex("div_filter", 'value');
				// 一行行插入数据
				for (var j = 0; j < jsonStr.length; j++) {
