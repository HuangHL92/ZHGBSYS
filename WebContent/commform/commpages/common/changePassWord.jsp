...
<odin:form action="/sys/userAction.do?method=alterPassword" target="hidden">
...
<iframe name="hidden" height="0" width="0" frameborder="0"> </iframe>
<script>
/** 首次载入列表数据开始 */
...
function ConfirmPassword(){
	  ...
	  var pd = document.all.newpasswd.value;
	  if(pd.length<6){
	  	odin.alert("用户密码不能少于6位！",odin.doFocus);
	    return false;
	  }
	  var confirm = document.all.confirm.value;
	  if(pd != confirm){
	  	odin.cueCheckObj = document.all.newpasswd;
...
function successFun(response){
    ...
    if(response.messageCode=='0'){ //修改成功
    	...
    	document.all.passwd.value = '';
       	odin.cueCheckObj = document.all.passwd;
    }
