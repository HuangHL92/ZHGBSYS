...
<link rel = "Shortcut Icon" href=icos/hzb.ico> 
...
<script type="text/javascript">
	...
	//密码校验专用窗口
	var openChangePassWin = function(id,title,url,width,height){
	    ...
	    winx = new Ext.Window({
	        html: "<iframe style=\"background:#cedff5;border:none;\" width=\"100%\" height=\"100%\" src=\"" + url + "\"></iframe>",
	        id:id,
	        title:title,
	        layout:'fit',
	        width:width,
...
<input type="hidden" id="chooseSystem" name="chooseSystem" value="1"/>
...
<div id="header_top" >
  ...
  <img src="image/iconfont-sanjiaoxing01.png" id="AB"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  <br/><div id="name" style="font-size: 13px;color: rgb(255,251,240);width: 200px; text-align:center;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;版本：<%=VerWindowBS.getVersionName() %></div>
	<div><a id="sw" href="javaScript:void(0)" style="font-size: 12px;color: rgb(255,251,240);width: 200px; text-align:center;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您有0条事务提醒</a></div>
  </td>
  </table>
  </div>
</div>	 	
...
<div id="bizarea"></div>
<div class="leftall" id="leftall" style="display: none;border:0px solid red;">
  <dl class="leftmenu" id="leftmenu" style="overflow:auto;overFlow-x: hidden;border:0px solid red;">
  </dl>
</div>
 ...
 <script type="text/javascript">
 	...
 	Ext.onReady(function(){	
 		...
 		if(checkPassWord == 'true'){
 			//大于30天 更换密码
 		var id = 'passwdWin';
 		openChangePassWin(id,'修改密码','<%=request.getContextPath()%>/radowAction.do?method=doEvent&pageModel=pages.sysmanager.user.PsWindow',document.body.clientWidth/2.8,document.body.clientHeight/2.4);
 		}
...
for(var i=0;i<data.length;i++){
	if(data[i].text=='公务员登记'||data[i].text=='表册输出'){
			...
			html += '<dd style="width:135%;background:url(./main/images/images/01_101.png) no-repeat;line-height:45px;font-weight:bold;font-size:14px;">'+
				'<div class="title" onclick="addTab1(&quot;'+data[i].id+'&quot;,&quot;'+data[i].text+'&quot;,&quot;'+data[i].location+'&quot;)"> <span><img src="'+data[i].icon+'" /></span>'+data[i].text+' </div>';
			html += '</dd>';
	}else {
			html += '<dd style="width:135%">'+
				'<div class="title"> <span><img src="'+data[i].icon+'" /></span>'+data[i].text+'</div>';
