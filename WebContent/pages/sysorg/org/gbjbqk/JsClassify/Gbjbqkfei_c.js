	...
	Ext.onReady(function openfile(){
	    try{
			document.getElementById("DCellWeb1").openfile(ctpath+"\\template\\gbjbqktjfei.cll","");
	    }catch(err){
	...
	var DCellWeb_tj="";
	var col_xho_start=4;
	var col_xho_end=34+4;
	var col_xhj_start=35+4;
	var col_xhj_end=45+4;
	//json字符串
   function json_func(){
	   //jsonstr,lengthstr
	  ...
	  for(var i=0;i<obj.length;i++){////插入数据 行数 循环
	   	   ...
	   	   var i_col=4;
	   		 DCellWeb_tj.SetCellString(
		   			4,a0221,0,obj[i].nv==0?"":obj[i].nv);//女 
		   		 i_col=i_col+2;
		   	DCellWeb_tj.SetCellString(
		   			i_col,a0221,0,obj[i].sm==0?"":obj[i].sm);//少数民族
		   /* i_col=i_col+2;		
		   	DCellWeb_tj.SetCellString(
		   			6,a0221,0,obj[i].zd==0?"":obj[i].zd);//中共党员 */
		   /* 	i_col=i_col+2;
		   	DCellWeb_tj.SetCellString(
		   			8,a0221,0,obj[i].fzd==0?"":obj[i].fzd);//非中共党员 */
		   	i_col=i_col+2;
		   	DCellWeb_tj.SetCellString(
