...
function myMouseDClick(col1, row1) {
	...
	//占比
	if(col1>=col_xho_start&&col1<=col_xho_end){//百分比 不反查
		if(col1%2==1){
			return;
		}
	}
	...
	//dwid=dwid+"$"+col1+"$"+row1+"$"+title;
	$h.openWin('GbjbqkxzccSub','pages.sysorg.org.gbjbqk.GbjbqkxzccSub',title+'-二维统计',1300,550,dwid,'<%=request.getContextPath()%>');
}
...
function CreateJson(value,name){// //JS 里面是不需要参数属性的 
	...
	function wenzi_func(){
		...
		// //干部基本情况分析
		wenzi_str="<br><p style='width:100%;text-align:center;'><b>干部基本情况分析(现职层次)</b></p><br><b>基本情况:</b><br><br>"
		wenzi_str=wenzi_str+"　　共(现职层次)<font onclick='opensubowin(\"3\",\"4\")' style='cursor:hand;color:#75acd2;'> <u>"+DCellWeb_tj.GetCellString(3, 4, 0)+"</u> </font>人 其中: ";
		var num_wz=0;
   ...
   function json_func(){
	   ...
	   try{
		   ...
		   alert("json数据格式有误，解析失败，请联系系统管理员修改数据!");
	   }
	   json_obj=obj;
	   ...
	   for(var i=0;i<obj.length;i++){////插入数据 行数 循环
	   	   ...
	   	   var i_col=4;
	   		DCellWeb_tj.SetCellString(
		   			4,a0221,0,obj[i].xzccxy1==0?"":obj[i].xzccxy1);//不满1年
		   	i_col=i_col+2;
		   	DCellWeb_tj.SetCellString(
		   			i_col,a0221,0,obj[i].xzccdy1==0?"":obj[i].xzccdy1);//1年
		   i_col=i_col+2;		
		   	DCellWeb_tj.SetCellString(
		   			i_col,a0221,0,obj[i].xzccdy2==0?"":obj[i].xzccdy2);//2年
		   	i_col=i_col+2;
		   	DCellWeb_tj.SetCellString(
		   			i_col,a0221,0,obj[i].xzccdy3==0?"":obj[i].xzccdy3);//3年
		   	i_col=i_col+2;
		   	DCellWeb_tj.SetCellString(
		   			i_col,a0221,0,obj[i].xzccdy4==0?"":obj[i].xzccdy4);//4年
		   	i_col=i_col+2;
		   	DCellWeb_tj.SetCellString(
		   			i_col,a0221,0,obj[i].xzccdy5==0?"":obj[i].xzccdy5);//5年
		   			i_col=i_col+2;
		   	DCellWeb_tj.SetCellString(
		   			i_col,a0221,0,obj[i].xzccdy6==0?"":obj[i].xzccdy6);//6年
  		   	i_col=i_col+2;
  		   	DCellWeb_tj.SetCellString(
  		   		i_col,a0221,0,obj[i].xzccdy7==0?"":obj[i].xzccdy7);//7年
  		   	i_col=i_col+2;
  		   	DCellWeb_tj.SetCellString(
  		   		i_col,a0221,0,obj[i].xzccdy8==0?"":obj[i].xzccdy8);//8年
  		   	i_col=i_col+2;
  		   	DCellWeb_tj.SetCellString(
  		   		i_col,a0221,0,obj[i].xzccdy9==0?"":obj[i].xzccdy9);//9年
  		   	i_col=i_col+2;
  		   	DCellWeb_tj.SetCellString(
  		   		i_col,a0221,0,obj[i].xzccdy10==0?"":obj[i].xzccdy10);//10年
  		   	i_col=i_col+2;
  		   	DCellWeb_tj.SetCellString(
  		   		i_col,a0221,0,obj[i].xzccdy11==0?"":obj[i].xzccdy11);//11年
 		   	i_col=i_col+2;
 		   	DCellWeb_tj.SetCellString(
 		   		i_col,a0221,0,obj[i].xzccdy12==0?"":obj[i].xzccdy12);//12年
			i_col=i_col+2;
			DCellWeb_tj.SetCellString(
					i_col,a0221,0,obj[i].xzccdy13==0?"":obj[i].xzccdy13);//13年
			i_col=i_col+2;
			DCellWeb_tj.SetCellString(
					i_col,a0221,0,obj[i].xzccdy14==0?"":obj[i].xzccdy14);//14年
			i_col=i_col+2;
			DCellWeb_tj.SetCellString(
					i_col,a0221,0,obj[i].xzccdy15==0?"":obj[i].xzccdy15);//15年
		    i_col=i_col+2;
		    DCellWeb_tj.SetCellString(
		    		i_col,a0221,0,obj[i].xzccdy16==0?"":obj[i].xzccdy16);//16年
	   		i_col=i_col+2;
	   		DCellWeb_tj.SetCellString(
	   				i_col,a0221,0,obj[i].xzccdy17==0?"":obj[i].xzccdy17);//17年
		   	i_col=i_col+2;
		   	DCellWeb_tj.SetCellString(
		   			i_col,a0221,0,obj[i].xzccdy18==0?"":obj[i].xzccdy18);//18年
		   	i_col=i_col+2;
		   	DCellWeb_tj.SetCellString(
		   			i_col,a0221,0,obj[i].xzccdy19==0?"":obj[i].xzccdy19);//19年
		   	i_col=i_col+2;
		    DCellWeb_tj.SetCellString(
		    		i_col,a0221,0,obj[i].xzccdy20==0?"":obj[i].xzccdy20);//20年
		   	//合计
			DCellWeb_tj.SetCellString(
  ...
  function zwlb_xy(){
	  ...
	  clear4();
	  if(zwlb_data=='undefined' || zwlb_data=="" || zwlb_data==null || zwlb_data==" "){//没有选择类别
		  if(xy_zwlb=='0'){
			  xsall();//显示所有行
		  }else{
  ...
  function displayzero_zwlb(start,end){
	  var v=0;
		 ...
		 if(v=='undefined' || v=="" || v==null||v==" "||v==0){
			 DCellWeb_tj.SetRowHidden(parseInt(start)-1,parseInt(start)-1);//隐藏小计
		 }
  }
  //清空第4行的数据
  function clear4(){
