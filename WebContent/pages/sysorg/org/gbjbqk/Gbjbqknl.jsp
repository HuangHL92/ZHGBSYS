...
<div id="createtable" style="overflow-y:hidden;overflow-x:hidden;overflow: auto;margin-left:15px;display:none;">
...
function myMouseDClick(col1, row1) {
	...
	//dwid=dwid+"$"+col1+"$"+row1+"$"+title;
	$h.openWin('GbjbqknlSub','pages.sysorg.org.gbjbqk.GbjbqknlSub',title+'-二维统计',1300,550,dwid,'<%=request.getContextPath()%>');
}
		...
		for(var i=col_xho_start+1;i<=col_xho_end;i++){//循环列
			   num_wz=DCellWeb_tj.GetCellString2(i, 4, 0);
				if(num_wz==0||num_wz==""||num_wz==null||num_wz==" "||num_wz=="undefined"){
					...
					if(col_xho_end==i){
						wenzi_str=wenzi_str+tjxm_arr[i]+"<font onclick='opensubowin(\""+i+"\",\"4\")' style='cursor:hand;color:#75acd2;'> <u>"+num_wz+"</u> </font>"+"岁 "
					}else{
		...
		if(v_temp_lb_zx==0||v_temp_lb_zx==""||v_temp_lb_zx==null||v_temp_lb_zx==" "||v_temp_lb_zx=="undefined"){
			...
			for(var j=col_xho_start+1;j<=col_xho_end;j++){//列数
				...
				if(num_wz==0||num_wz==""||num_wz==null||num_wz==" "||num_wz=="undefined"){
					...
					if(col_xho_end==j){
						wenzi_str=wenzi_str+tjxm_arr[j]+"<font onclick='opensubowin(\""+j+"\",\""+parseInt(start-1)+"\")' style='cursor:hand;color:#75acd2;'> <u>"+num_wz+"</u> </font>"+"岁 "
					}else{
			...
			if(v_temp_lb_zx==0||v_temp_lb_zx==""||v_temp_lb_zx==null||v_temp_lb_zx==" "||v_temp_lb_zx=="undefined"){
				...
				for(var j=col_xho_start+1;j<=col_xho_end;j++){//列数
					...
					if(num_wz==0||num_wz==""||num_wz==null||num_wz==" "||num_wz=="undefined"){
						...
						if(col_xho_end==j){
							wenzi_str=wenzi_str+tjxm_arr[j]+"<font onclick='opensubowin(\""+j+"\",\""+i+"\")' style='cursor:hand;color:#75acd2;'> <u>"+num_wz+"</u> </font>"+"岁 "
						}else{
