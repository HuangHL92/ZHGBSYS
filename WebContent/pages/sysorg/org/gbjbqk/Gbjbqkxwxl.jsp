...
function myMouseDClick(col1, row1) {
	...
	//dwid=dwid+"$"+col1+"$"+row1+"$"+title;
	$h.openWin('GbjbqkxwxlSub','pages.sysorg.org.gbjbqk.GbjbqkxwxlSub',title+'-二维统计',1300,550,dwid,'<%=request.getContextPath()%>');
}
 // //文字单击事件 
...
function CreateJson(value,name){// //JS 里面是不需要参数属性的 
	...
	function wenzi_func(){
		...
		wenzi_str=wenzi_str+"　　共(学历学位)<font onclick='opensubowin(\"3\",\"4\")' style='cursor:hand;color:#75acd2;'> <u>"+DCellWeb_tj.GetCellString(3, 4, 0)+"</u> </font>人 其中: ";
		var num_wz=0;
  ...
  //显示所有行
  function xsall(){
	DCellWeb_tj.SetRowUnhidden(4, array_row[array_row.length-1].split(",")[1]);
  }
