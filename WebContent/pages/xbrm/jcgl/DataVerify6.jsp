...
<form name="excelForm" id="excelForm" method="post"  action="<%=request.getContextPath()%>/RegCheckServlet?method=impPCXXFile" enctype="multipart/form-data" >	
...
function formupload(){
	...
	if(!(type.toUpperCase()=='ZIP')){
		alert("不是ZIP格式文件，不能导入！");
		return;
	}
	parent.Ext.Msg.wait('正在上传，请稍候...','系统提示：');
	odin.ext.Ajax.request({
		url:'<%=request.getContextPath()%>/RegCheckServlet?method=impPCXXZIPFile',
		isUpload:true,
		form:'excelForm',
		success:function(){
			parent.Ext.Msg.hide();
