...
$(document).ready(function () {
	...
	Ext.Ajax.request({
		url:'<%=request.getContextPath()%>/RegCheckServlet?method=getXXK&fileid='+fileid,
		isUpload: false,
		form:'excelForm',
		success:function(){
			parent.Ext.Msg.hide();
...
function deletefj(id){
	if(confirm("你确定要删除已上传的附件吗？")){
		...
		odin.ext.Ajax.request({
			url:'<%=request.getContextPath()%>/RegCheckServlet?method=deleteXXKFile&fid='+id,
			isUpload:true,
			form:'excelForm',
			//params : {'fid': id},
			success:function(){
...
function formupload(){
	...
	odin.ext.Ajax.request({
		url:'<%=request.getContextPath()%>/RegCheckServlet?method=impXXK&fileid='+fileid,
		isUpload:true,
		form:'excelForm',
		success:function(){
			parent.Ext.Msg.hide();
