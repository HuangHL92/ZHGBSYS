...
<div id="bottomDiv" style="width: 100%;">
			...
			<tr align="center">
				<td>
					<table>
						<tr>
							<td><div onclick="clearAll()" class="QueryBtn">清空</div>
						</td>
						<td><div onclick="Query()" class="QueryBtn">查询</div>
							</td>
						<tr>
					</table>
				</td>
...
function Query(){
    ...
    for (var i = 0; i < inputs.length; i++) {
        if (inputs[i].type == "checkbox") {
			var inp = null;
        	if(inp=inputs[i].checked){
        		param = param+inputs[i].id+",";
        	}
        }
    } 
    if(param==""){
    	$h.alert('系统提示：','请勾选一项！',null,180);
    	return;
    }else{
    	param=param.substring(0,param.length-1);
    }
	radow.doEvent('queryData',param);
}
function clearAll(){
	var inputs = document.getElementsByTagName("input");
    for (var i = 0; i < inputs.length; i++) {
...
Ext.onReady(function(){
	if(window.realParent.document.getElementById("fields").value==""){
		return;
	}
	var fields = window.realParent.document.getElementById("fields").value.split(",");
