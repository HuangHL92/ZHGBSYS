<%@page import="com.insigma.siis.local.pagemodel.bzpj.PJGLInfo"%>
...
var coldata = <%=PJGLInfo.getColData(egl00)%>; 
var gradedata = <%=PJGLInfo.getGradeData(egl00)%>;
var b01data = <%=PJGLInfo.getB01Data(egl00)%>;
var GLOBLE = {};
GLOBLE['colspan_info']={};
//行机构id
GLOBLE['ROWID']=[]
	...
	$.each(coldata, function (i,item) {
		if(GLOBLE['colspan_info'][item["etc00"]]){
			...
			if(i==0){
				...
				GLOBLE['colspan_info']['et00'] = GLOBLE['colspan_info']['et00']+1+1;
			}else{
...
function ajaxSubmit(radowEvent,parm,callback){
	  ...
	  Ext.Ajax.request({
	    ...
	    url: "<%=request.getContextPath()%>/radowAction.do?method=doEvent&pageModel=pages.bzpj.PJGL&eventNames="+radowEvent,
