...
<div id="tag_container">
	...
	<iframe src ="<%=request.getContextPath()%>/pages/zj/tags/addTagNdkhdjbfj.jsp" id="wzms" frameborder="0"></iframe>
	<div id="grid">
		<table>
			<tr>
				<td width="100%">
					<odin:grid property="tagNdkhdjbfjGrid" sm="row"  forceNoScroll="true" isFirstLoadData="false" url="/" height="300">
...
Ext.onReady(function(){
	if(realParent.buttonDisabled){
		...
		var ext_gridobj = Ext.getCmp('tagNdkhdjbfjGrid');
		var gridobj = document.getElementById('forView_tagNdkhdjbfjGrid');
...
Ext.onReady(function(){
	var side_resize=function(){
		 Ext.getCmp('tagNdkhdjbfjGrid').setWidth(810); 
	}
	side_resize();  
...
function GrantRender(value, params, rs, rowIndex, colIndex, ds){
	return "<a href=\"javascript:downloadNdkhdjbfjFile('"+rs.get('tagid')+"','"+rs.get('fileurl')+"')\">下载</a>" + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + 
	"<a href=\"javascript:deleteNdkhdjbfjFile('"+rs.get('tagid')+"','"+rs.get('fileurl')+"')\">删除</a>" + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
}

// 年度考核登记表附件下载 
// encodeURI，用来做url转码，解决中文传输乱码问题 （后台接收的时候会再做转码处理，转回来）
function downloadNdkhdjbfjFile(tagid, fileurl){
	window.location="<%=request.getContextPath()%>/TagFileServlet?method=downloadNdkhdjbfjFile&fileurl="+encodeURI(encodeURI(fileurl));
}

//年度考核登记表附件删除
//encodeURI，用来做url转码，解决中文传输乱码问题 
function deleteNdkhdjbfjFile(tagid, fileurl){
	//通过ajax请求，删除年度考核登记表附件 
	$.ajax({
		url:"<%=request.getContextPath()%>/TagFileServlet?method=deleteNdkhdjbfjFile",
		type:"GET",
		data:{
			"tagid": tagid,
		...
		success:function(){
			radow.doEvent('tagNdkhdjbfjGrid.dogridquery');
			updateRmbFresh();
			odin.alert("年度考核登记表附件删除成功!");
		},
		error:function(){
			odin.alert("年度考核登记表附件删除失败!");		
		}
	});
}
