...
<odin:base>
<form name="excelForm" id="excelForm" method="post" action="<%=request.getContextPath()%>/TagFileServlet?method=addDazsbfjFile" enctype="multipart/form-data" >	
...
function formSubmit(){
	...
	if(file !=""){
		...
		odin.ext.Ajax.request({
			url: '<%=request.getContextPath()%>/TagFileServlet?method=addDazsbfjFile&a0000='+a0000+'&note='+encodeURI(note),
			isUpload: true,
			method: 'post',
			fileUpload: true,
			form: 'excelForm',
			success: function(){
				...
				parent.odin.ext.getCmp('tagDazsbfjGrid').store.reload();
			}
		});
	}else{
		odin.info('文件必须上传！');
