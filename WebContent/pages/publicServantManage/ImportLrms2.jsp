...
<odin:base>
	...
	<form name="excelForm" id="excelForm" method="post"  action="<%=request.getContextPath()%>/servlet/UpLoadLrmsServlet" enctype="multipart/form-data" target="impFrame">
		<table >
			...
			<tr>
				<odin:select2   property="TPHJ"  required="true" codeType="KZTPHJ02"  width="330" label="批次选择"></odin:select2>
			</tr>
			<tr>
				<td></td>
...
//	var pic =document.getElementById('pic').value;
        ...
        function updateUpLoad(value){
            var a0201b = document.getElementById('a0201b').value;
            var TPHJ = document.getElementById('TPHJ').value;
            $h.confirm3btn('系统提示（重要）','如果系统发现任免表人员与本库人员信息重复，是否还执行追加？<br/>'+
                ...
                if(id=='yes'){
                    ...
                    Ext.Ajax.request({
                        url : "<%=request.getContextPath()%>/UpLoadLrmsServlet?method=Again&a0201b="+a0201b+"&TPHJ="+TPHJ,
                        isUpload : true,
                        form : "excelForm",
                        success : function(response) {
                            eval(response.responseText);
                    ...
                    Ext.Msg.wait("正在导入，请稍后...","系统提示");
                    Ext.Ajax.request({
                        url : "<%=request.getContextPath()%>/UpLoadLrmsServlet?a0201b="+a0201b+"&TPHJ="+TPHJ,
                        isUpload : true,
                        form : "excelForm",
                        success : function(response) {
                            eval(response.responseText);
