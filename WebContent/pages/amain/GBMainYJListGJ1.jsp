	...
	<odin:editgrid2 property="noticeSetgrid" hasRightMenu="false" autoFill="false" 
	 ...
	 bbarId="pageToolBar" pageSize="50" isFirstLoadData="false" url="/">
		...
		<odin:gridColumnModel>
			...
			<odin:gridEditColumn2 dataIndex="a0192a" width="600" header="任现职务" align="center" editor="text" edited="false" />
			<odin:gridEditColumn2 dataIndex="zxs" width="200" header="培训时长" align="center" editor="text" edited="false" isLast="true"/>
		</odin:gridColumnModel>
	</odin:editgrid2>
</div>
...
Ext.onReady(function() {
	...
	if(type=='edugzmd'){
		pgrid.colModel.getColumnAt(4).header='网络学时';
		pgrid.reconfigure(pgrid.store,pgrid.colModel);
	}else if(type=='edu36'){
		pgrid.colModel.getColumnAt(4).header='培训天数';
		pgrid.reconfigure(pgrid.store,pgrid.colModel);
	}else if(type=='fc5t'){
		pgrid.colModel.getColumnAt(4).header='持有房产数量';
		pgrid.reconfigure(pgrid.store,pgrid.colModel);
	}else if(type=='jsb2000w'){
		pgrid.colModel.getColumnAt(4).header='经商办企业人员';
		pgrid.reconfigure(pgrid.store,pgrid.colModel);
	}else if(type=='jttz2000w'){
		pgrid.colModel.getColumnAt(4).header='身份证号';
		pgrid.reconfigure(pgrid.store,pgrid.colModel);
	}
