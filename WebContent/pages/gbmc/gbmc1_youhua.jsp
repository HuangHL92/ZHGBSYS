...
<%@page import="com.insigma.siis.local.pagemodel.gbmc.pojo.Gbmc1"%>
...
Map<String, List<Gbmc1>> gruopMap = ExpTPB.querybyid_gbmc(sql);
%>
<html>
<head>
 <meta charset="utf-8">
...
<% }else{ 
	Integer rows = 5; //固定行数
	Integer index = 0;//页码
	Set<Entry<String, List<Gbmc1>>> entrySet = gruopMap.entrySet();
	  
	  for (Entry<String, List<Gbmc1>> entry : entrySet) {
		//机构主键&机构名
		  String key = entry.getKey();
		//机构名
		  String dept = key.split("&")[1];
	    //人员信息集
		  List<Gbmc1> list = entry.getValue();
	    //该单位的页数
	    int index1 = 0;
		  for (int i = 0 ; i<list.size() ; i++) {
			  Gbmc1 gbmc = list.get(i);
