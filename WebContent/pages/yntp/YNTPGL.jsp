...
<odin:editgrid2 property="memberGrid" load="selectRow" hasRightMenu="false" title="批次信息" autoFill="true"  bbarId="pageToolBar"  url="/">
	...
	<odin:gridColumnModel>
		...
		<odin:gridEditColumn2 dataIndex="info01" width="190" header="共享信息" editor="text" edited="false" align="center" renderer="infoDesc" isLast="true"/>
...
<odin:toolBar property="btnToolBar" applyTo="groupTreePanel">
	...
	<odin:buttonForToolBar text="模拟方案读取" icon="image/icon021a2.gif" id="mntpadd" handler="mntpadd"/>
...
function loadadd(){
	$h.openPageModeWin('loadadd','pages.yntp.AddYNTP','增加批次',510,120,{yn_id:''},g_contextpath);
}
...
function infoUpdate(){
	...
	$h.openPageModeWin('loadadd','pages.yntp.AddYNTP','修改批次',510,120,{yn_id:yn_id},g_contextpath);
}
...
function mntpadd(){
	...
	$h.openWin('MN2YN','pages.yntp.MN2YN','酝酿调配方案',700,500,'','<%=request.getContextPath()%>',window,p,true);
}
function openGDview(yn_id,w){
	w.close();
	var newWin_ = Ext.getCmp('MN2YN');
	if(newWin_){
		//newWin_.destroy(); 
	}
	//infoSearch();
