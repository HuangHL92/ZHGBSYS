...
function formSubmit2(){
	...
	if(type2 == "MYSQL" && Number(count2)+Number(psncount1)>100000){
		...
		Ext.Ajax.request({
				url:'<%=request.getContextPath()%>/UploadFileServlet?method=zzbFileImpPsnZzb&uuid'+uuid+'&filename'+filename,
...
///		parent.doOpenPupWin("/radowAction.do?method=doEvent&pageModel=pages.repandrec.local.KingbsWinfresh&initParams="
///								+ uuid, "导入详情", 600, 470, null);
		...
		Ext.Ajax.request({
				...
				success:function(){
					realParent.odin.ext.getCmp('MGrid').store.reload();
					parent.Ext.getCmp(subWinId).close();
					//'realParent.odin.ext.getCmp('simpleExpWin2').hide();
					///FileUpload_onselect();
					///Ext.Msg.alert('upfile','文件上传成功！');
				}
			});
