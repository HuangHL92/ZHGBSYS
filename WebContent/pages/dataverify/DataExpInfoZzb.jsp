...
<div>
 ...
 <script type="text/javascript">  
	...
	function dataExp(){
		$h.openWin('dataOrgImp','pages.dataverify.DataOrgImpZzb','HZB格式数据上报窗口',750,550, '','<%=request.getContextPath()%>');
	}
	function unidataExp(){
		$h.openWin('dataOrgImpUni','pages.dataverify.DataOrgImpUniZzb','通用格式数据上报窗口',750,550, '','<%=request.getContextPath()%>');
	}
	function exceldataExp(){
		$h.openWin('dataOrgExcelExp','pages.dataverify.DataExcelExportZzb','Excel格式数据上报窗口',750,200,'','<%=request.getContextPath() %>');
	}
...
<script type="text/javascript">
     Ext.onReady(function () {
         ...
         var openMsgWin = new Ext.Window({
             closable: false,draggable:false,resizable:false,closable: true,closeAction:'hide',autoHide:3,shadow: false, resizable: false, 
             x: eBody.getWidth() - msgWinConfig.width, y: eBody.getHeight() - msgWinConfig.height, width: msgWinConfig.width, height: msgWinConfig.height
             , items: [{ xtype:"textfield", width: 300, height: 150,readOnly: true,
            	 style: "font-family: 'palatino linotype', palatino, serif; color: #EB2626; font-size: 16px; font-weight: bold; word-spacing: 4pt;",
            	 value:"备份请选用HZB格式导出进行备份。" }]
             , flyIn: function () {
