...
<script type="text/javascript">
	function impTestOld() {//导入老版本数据
		$h.openWin('impNewHzbORGOLDWin', 'pages.dataverify.DataVerifyNewOldHZB', '按机构导入数据窗口', 850, 550, '', '<%=request.getContextPath()%>');
	}
	
	function impTestMake() {//导入套改版本数据
		$h.openWin('impNewHzbORGMakeWin', 'pages.dataverify.DataVerifyNewMakeHZB', '按机构导入数据窗口', 850, 550, '', '<%=request.getContextPath()%>');
	}
...
////			$h.openWin('impNewHzbORGWin', 'pages.dataverify.DataVerifyNew','导入窗口',850,550,'1111@1111', '');
		$h.openWin('impNewHzbORGWin', 'pages.dataverify.DataVerifyNewZzb', '按机构导入数据窗口', 850, 550, '', '<%=request.getContextPath()%>');
					
	}
	function TYGSimp() {
		$h.openWin('impNewHzbORGWin', 'pages.tygsImp.DataVerifyNew2Zzb', '通用格式导入数据窗口', 850, 550, '', '<%=request.getContextPath()%>');
	}
	function Excelimp() {
		$h.openWin('ExcelImp', 'pages.customquery.ExcelImpZzb', 'Excel导入数据窗口', 400, 430, '', '<%=request.getContextPath()%>');
	}
	function Mysqlimp() {
		$h.openWin('impNewMysqlWin', 'pages.customquery.MysqlImp', 'Mysql库接收', 850, 550, '', '<%=request.getContextPath()%>');
	}
	function impTest2() {
...
///	win.setTitle('导入窗口');
	...
	$h.showWindowWithSrc('simpleExpWin2', contextPath
				+ "/pages/dataverify/DataVerify3Zzb.jsp?i=1",'导入窗口',530,250);	
	}
