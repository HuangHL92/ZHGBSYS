...
document.getElementById('LWflag').value = realParent.document.getElementById('LWflag').value;
	...
	<odin:tab id="tab"  tabchange="grantTabChange">
   		<odin:tabModel>
       <odin:tabItem title="基本信息" id="tab1" isLast="true"></odin:tabItem>     
		...
		<div id="jbDiv">
			<table style="width: 100%;height: 530">
				<tr>
					...
					<odin:select2 property="b0131A" label="机构类别" data="['01', '市直党委部门'],['02', '市直政府部门'],['03', '区县市党委部门'],['04', '区县市政府部门'],['05', '镇'],['06', '乡'],['07', '街道']" multiSelect="true"></odin:select2>
 --%>
					
										
				</tr>
				<tr>
					<odin:select2 property="a0165A" label="省管干部" data="['02', '省管干部']" multiSelect="true"></odin:select2>
					<odin:select2 property="a0165B" label="市管干部" data="['10', '市管正职'],['11', '市管副职'],['19', '市管职级公务员（含退出领导岗位）']" multiSelect="true"></odin:select2>
					<odin:select2 property="a0165C" label="处级（中层）干部" data="['12', '市直正处'],['07，09，10', '区、县（市）正处（中层正职）'],['14','国企高校正职']" multiSelect="true"></odin:select2>
				</tr>
			<!-- 	<tr>
					<td noWrap="nowrap" align=right><span id=sexSpanId style="FONT-SIZE: 12px">性&nbsp;&nbsp;&nbsp;别</span>&nbsp;</td>
					<td colspan="1" style="FONT-SIZE: 12px">
				...
				<tr>
					<odin:select2 property="nljy" label="年 &nbsp;&nbsp;&nbsp;&nbsp;龄" data="['30', '30岁以下'],['35', '35岁以下'],['40','40岁左右及以下' ],['45','45岁左右及以下'],['55','55岁以上']" multiSelect="false"></odin:select2>
					<odin:select2 property="csnyjy" label="出生年月" data="['45', '75后'],['40', '80后'],['35','85后' ],['30','90后']" multiSelect="false"></odin:select2>
					<odin:select2 property="a0165D" label="副处（中层）" data="['50', '市直副处'],['08,13', '区、县（市）副处（中层副职）'],['15', '国企高校副处']" multiSelect="true"></odin:select2>
				...
				<tr>
					<odin:select2 property="xlxw" label="学 &nbsp;&nbsp;&nbsp;&nbsp;历" data="['博士', '博士'],['研究生', '研究生'],['大学,本科','大学' ],['大学,本科,硕士,研究生,博士', '全日制大学以上']" multiSelect="false"></odin:select2>
					<odin:select2 property="zdgwq" label="重要任职经历" data="['L0301', '乡镇（街道）党政正职'],['L0302', '乡镇街道党委书记'],['L0201,L0202,L0203,L0204,L0205,L0206,L0207,L0208,L0209,L0210,L0211,L0212,L0213,L0214,L0215,L0216','区县市党政班子成员' ]
					,['L0205,L0206,L0207', '区县市重要岗位（副书记，兼任大镇街书记的常委，政府常务）'],['L0103', '市直单位双副'],['L0603', '副师职以上军转'],['L0701,L0702', '援派挂职']" multiSelect="false"></odin:select2>
					<!-- <td><label style="FONT-SIZE: 12px">全日制</label><input name="qrz" id="qrz" type="checkbox" value="1" /> </td> -->
				...
				<%-- <tr>
					...
					<%TableColInterface userBs  = new TableColInterface(); if ( userBs.isA0194Tags() ) {
					%>
					<tags:ComBoxWithTree property="A0194_TAG" label="熟悉领域" codetype="SXLY2" readonly="true" ischecked="true" width="160"></tags:ComBoxWithTree>
					<%
   					 }
...
<!-- 					<td  align="center"><label style="FONT-SIZE: 12px">是否为现任</label><input name="sfwxr" id="a" type="checkbox" value="1" /></td>
 -->				
				</tr> --%>
...
function chosePerson(){
	...
	//var type = $("#")
$h.openPageModeWin('rewardPunish','pages.customquery.PersonSelected&LWflag='+LWflag,'人员选择',810,370,111,contextPath,null,{maximizable:false,resizable:false})
	}
function receiveId(a0000,a0101){
	$('#ppA0000').val(a0000);
	$('#ppName').val(a0101);
...
/* 	//zb09标红
	var zb09Word1=document.getElementById('zb09Word').value; //获取本页面的值
	alert(zb09Word1);
	realParent.document.getElementById("zb09Words").value=zb09Word1;   //向父页面赋值 */
...
function clearConPP(){
	...
	document.getElementById('ppA0000').value="";
}
...
function gbmcQueryBtn(){
	realParent.$h.openPageModeWin('gbmcQuery','pages.customquery.gbmcQuery','干部查询列表',650,480,'','<%=request.getContextPath()%>',window);
}

realParent.groupWin = window;
</script>
