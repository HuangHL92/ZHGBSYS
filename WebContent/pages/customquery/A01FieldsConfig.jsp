...
<odin:toolBar property="btnToolBar" applyTo="divbtnToolBar">
  <odin:fill />	
	<odin:buttonForToolBar id="tableBtn" text="切换方案" menu="tableMenu"
		icon= "icon/refresh.gif" cls="x-btn-text-icon"/>
		
	<odin:buttonForToolBar id="iconToolBarBtnRefresh" text="重置" handler="reload"
		icon= "icon/refresh.gif" cls="x-btn-text-icon"/>
		
	<odin:buttonForToolBar id="iconToolBarBtnSave" isLast="true" text="确定" handler="submitForm"
		icon="icon/save.gif" cls="x-btn-text-icon" />
...
<div id="tree-div" style="overflow-x:hidden; overflow-y:scroll; border: 2px solid #c3daf9; height: 400px; font-size: 16px;padding-bottom: 60px;">
...
function submitForm(){
	...
	Ext.Ajax.request({
        ...
        //timeout :30000,//按毫秒计算
		url: "<%=request.getContextPath()%>/radowAction.do?method=doEvent&pageModel=pages.customquery.CustomQuery&eventNames=saveA01_config",
		success: function(resData){
			...
			/* Ext.Msg.alert('系统提示',cfg.mainMessage,function(id){
				if(0==cfg.messageCode){
					var realParent = parent.Ext.getCmp("setFields").initialConfig.thisWin;
					//parent.document.location.reload();
					//alert(cfg.elementsScript);
...
Ext.onReady(function() {
    ...
    var tree = new Tree.TreePanel( {
        ...
        /* tbar:new Ext.Toolbar({
     		  ...
     		  xtype: "button",
              text : "重置",
              iconCls:"iconToolBarBtnRefresh",
              handler:reload,
              id:'isContain'
            },'-',
            {   text: "button",
            	text : "确定",
            	iconCls:"iconToolBarBtnSave",
            	handler:submitForm,
                id:'ltry'
            },new Ext.Spacer({width:30})]
     //height:45,
     //layout :'table'
     }
        ), */
        ...
        loader : new Tree.TreeLoader( {
      	  ...
      	  dataUrl : '<%=request.getContextPath()%>/radowAction.do?method=doEvent&pageModel=pages.customquery.CustomQuery&eventNames=a01_config_tree'
        }),
        listeners:{
    ...
    treed.style.height = viewSize.height-0;
    ...
    Ext.tree.TreeCheckNodeUI = function() { 
    	...
    	<odin:menu property="tableMenu">
		<odin:menuItem text="方案一" property="printOne"  handler="printOne" ></odin:menuItem>
		<odin:menuItem text="方案二" property="printTwo"  handler="printTwo" ></odin:menuItem>
		/* <odin:menuItem text="方案三" property="printTre"  handler="printtre" ></odin:menuItem>
		<odin:menuItem text="方案四" property="printFor"  handler="printfor" ></odin:menuItem> */
		</odin:menu>
		...
		function printType(type){
			...
			if(type=='1'){
				Ext.Ajax.request({
			        ...
			        async: true,
			        params : {'type':type},
					url: "<%=request.getContextPath()%>/radowAction.do?method=doEvent&pageModel=pages.customquery.CustomQuery&eventNames=saveA01_config_T",
					success: function(resData){
						var cfg = Ext.util.JSON.decode(resData.responseText);
						if(0==cfg.messageCode){
							var realParent = parent.Ext.getCmp("setFields").initialConfig.thisWin;
