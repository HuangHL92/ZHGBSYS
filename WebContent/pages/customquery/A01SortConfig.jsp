...
<div id="tree-div" style="overflow-x:hidden; overflow-y:scroll; border: 2px solid #c3daf9; height: 400px; font-size: 16px;"></div>
</form>
...
function submitForm(){
	...
	for(var i=0;i<childNodes.length;i++){
		...
		var orderTypeId = node.id+"orderType";
		
		
		jsonp[node.id+"orderType"] = document.getElementById(orderTypeId).checked;
	...
	Ext.Ajax.request({
        ...
        //timeout :30000,//按毫秒计算
		url: "<%=request.getContextPath()%>/radowAction.do?method=doEvent&pageModel=pages.customquery.CustomQuery&eventNames=saveA01_sortConfig",
		success: function(resData){
			
			//调用父页面方法 ,刷新列表 
			realParent.load();
			//关闭本页面 
			parent.odin.ext.getCmp('sortHand').close();
...
Ext.onReady(function() {
    ...
    var tree = new Tree.TreePanel( {
     ...
     //layout :'table'
        ...
        loader : new Tree.TreeLoader( {
      	  ...
      	  dataUrl : '<%=request.getContextPath()%>/radowAction.do?method=doEvent&pageModel=pages.customquery.CustomQuery&eventNames=a01_sort_tree'
        }),
        listeners:{
    ...
    Ext.tree.TreeCheckNodeUI = function() { 
    	...
    	'<input class="x-tree-node-cb" id="',n.id,'orderType" type="checkbox" ' + (a.orderType ? 'checked="checked" />' : '/>'), 		
