...
<form name="excelForm" id="excelForm" method="post"  action="<%=request.getContextPath()%>/OtherdbServlet?method=impDxscgFile" enctype="multipart/form-data" >
...
function formupload(){
	...
	odin.ext.Ajax.request({
		url:'<%=request.getContextPath()%>/OtherdbServlet?method=impDxscgFile',
		isUpload:true,
		form:'excelForm',
		success:function(){
			parent.Ext.Msg.hide();
