		...
		onLoad : function (panel) {
			...
			if(tab.panel('options').title == '任免表（一）'){
				...
				if($.browser.msie){
					...
					var remarkV = document.getElementById("remark").value.replaceAll("\r\n","<br/>");
					editor_a.render('remark');
					editor_a.setContent(remarkV);
					editor_a.setHeight(182);
				}else{
					var remarkV = document.getElementById("remark").value.replaceAll("<p>","").replaceAll("</p>","").replaceAll("<br />","\n");
					document.getElementById("remark").value = remarkV;
				}
				System.report.menu("#btnPrint",function(){
					System.report.print($("#personcode").val());
				},'b');
				$.get(System.rootPath+"/common/exec-common-method!checkPermission.action?serviceid="+$("#serviceid").val()+"&personcode="+$("#personcode").val()+"&currentUnit="+$("#currentUnit").val(),function(data){
					var flag = false;
					if(data.unitcode){
				...
				$.get(System.rootPath+"/common/exec-common-method!getEditUnits.action?serviceid="+$("#serviceid").val()+"&personcode="+$("#personcode").val(),function(data){
					var flag = false;
					if(data.unitcode){
						$("#unitcode").val(data.unitcode);
						if(data.editable){
					...
					if(flag){
						$("#jianglqk").attr("disabled",false);
						$("#niandkh").attr("disabled",false);
					}else{
...
var Removal = {
	...
	orderFamily:function(){
		...
		$("#w").window({
			...
			height:400,
			title:'家庭成员排序',
			content:"<iframe border='0' width='100%' height='99%' frameborder='0' src='"+System.rootPath+"/removal/removal!familyOrder.action?personcode="+personcode+"'></iframe>"
		});
	},
	orderFamilyCallback:function(){
		$("#jtcy").datagrid('reload');
	},
	xl:function (target,type){
		...
		$("#w").window({
			...
			height:300,
			title:'编辑学历信息',
			content:"<iframe border='0' width='100%' height='99%' frameborder='0' src='"+System.rootPath+"/xlxw/xl-modify-show?personcode="+personcode+"&xltype="+type+"'></iframe>"
		});
	},
	xw:function (target,type){
		...
		$("#w").window({
			...
			height:300,
			title:'编辑学位信息',
			content:"<iframe border='0' width='100%' height='99%' frameborder='0' src='"+System.rootPath+"/xlxw/xw-modify-show?personcode="+personcode+"&xltype="+type+"'></iframe>"
		});
	},
	zhiw:function(target){
		...
		$("#w").window({
			...
			height:300,
			title:'编辑职务信息',
			content:"<iframe border='0' width='100%' height='99%' frameborder='0' src='"+System.rootPath+"/zw/zw-modify-show?personcode="+personcode+"'></iframe>"
		});
	},
	kh:function (target){
		var personcode = $("#personcode").val();
		$("#w").window({
			...
			height:400,
			title:'编辑年度考核结果',
			content:"<iframe border='0' width='100%' height='99%' frameborder='0' src='"+System.rootPath+"/kh/kh-modify-show?personcode="+personcode+"'></iframe>",
			onClose : function (){
				getKhInfo();
			}
		});
	...
	jc:function (target){
		...
		$("#w").window({
			...
			height:300,
			title:'编辑奖惩情况',
			content:"<iframe border='0' width='100%' height='99%' frameborder='0' src='"+System.rootPath+"/jc/jc-modify-show?personcode="+personcode+"'></iframe>",
			onClose : function (){
				getJcInfo();
			}
		});
