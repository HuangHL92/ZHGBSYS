...
function orderRecord(order) {
	var url = '';
	if (g_isView == '0') url = System.rootPath + '/ndkh/ndkh!cityUnitPersonBzfzNdkhjg.action'; 
...
function showPersonSjcp(unitId, personcode) {
	var pWin = getParent();
	if (pWin != null) {
		pWin.openCityUnitPersonSjcpWin(unitId, personcode, g_year, g_formType);
	}
}
