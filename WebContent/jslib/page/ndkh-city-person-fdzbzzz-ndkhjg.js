...
function orderRecord(order) {
	var url = '';
	if (g_isView == '0') url = System.rootPath + '/ndkh/ndkh!cityUnitPersonFdzbzzzNdkhjg.action'; 
...
function showPersonMzcp(unitId, personcode) {
	var pWin = getParent();
	if (pWin != null) {
		pWin.openCityUnitPersonZxrdqtMzcpWin(unitId, personcode, g_year, g_formType);
	}
}
