var UnitPicker = function(options) {
	var _self = this; // 保留自身，其他组件里面的事件，必须用_self代替this
	
	this.unitType = options.unitType;
	this.dialogId = 'pickUnitDlg';
	this.unitTreeId = 'pickUnitTree';
	this.unitSearchBox = 'pickUnitSearchBox';
	...
	// 初始化
	UnitPicker.prototype.init = function() {
		_self._init();
	};
	
	// 选择单位
	UnitPicker.prototype.pickUnit = function() {
		_self._pickUnit();
	};
	...
	this._initPicker = function() {
		if ($('#' + _self.dialogId).length <= 0) {
			var content = "<div id='pickUnitDlg'>";
			$("body").append(content);	
		}
		...
		if (!_self.multiple) {
			...
			content += "按单位名称快速搜索：<input id='pickUnitSearchBox' iconCls='icon-search' style='width:350px;' prompt='输入姓名或者姓名首字母拼音进行搜索'/>";
			content += "</div>";
		}
		content += "<div region='center' border='false'>";
		content += "<ul id='pickUnitTree' class='ztree'></ul> ";
		content += "</div>";
		content += "</div>";
		content += "</div>";
	...
	_self._getButtons = function() {
		if (!_self.multiple) {
			return [{
				...
				iconCls: 'icon-ok',
				handler: function(){
					var zTree = getZTreeObj('pickUnitTree');
					var nodes = zTree.getSelectedNodes();
					if (nodes.length >= 1) {
						var unit = { id: nodes[0].id, name : nodes[0].text};
			...
			},
			{
				...
				handler: function(){
					...
					if (nodes) {
						$(nodes).each(function(index, node) {
							if (!node.isParent) {
							    var unit = { id: node.id, name: node.text };
							    units.push(unit);
							}
						});
	...
	this._initUnitTree = function() {
		...
		if (_self.multiple) {
			checkSetting = {
				...
				chkboxType: { "Y" : "s", "N" : "s" } // 勾选节点，不影响父节点和子节点
			};
		}
		
		$("#" + _self.unitTreeId).UnitTree({
