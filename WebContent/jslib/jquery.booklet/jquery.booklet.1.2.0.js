		...
		if(opts.pageSelector){
			...
			for(i=0; i < opts.pTotal; i+=2){
				...
				if(opts.direction == 'RTL'){
					...
					ddLI = $('<li><a href="#/page/'+ (i+1) +'" id="selector-page-'+i+'"><span class="b-text">'+ titles[i+1] + " - " + titles[i + 2] +'</span><span class="b-num">'+ nums +'</span></a></li>').prependTo(ddUL);
				}else{
					if(i==0){dd.find('.b-current').text(nums);}
					ddLI = $('<li><a href="#/page/'+ (i+1) +'" id="selector-page-'+i+'"><span class="b-text">'+ titles[i] + " - " + titles[i + 1] + '</span><span class="b-num">'+ nums +'</span></a></li>').appendTo(ddUL);
				}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$.extend(self,
	{
		next : function(){
			if(!busy){
				// YZQ ADD -- Start
				//if (opts.curr == 0) $("#bwrapperDiv").removeClass("bwrapper1").addClass("bwrapper2");
				//else if (opts.curr == 4) $("#bwrapperDiv").removeClass("bwrapper2").addClass("bwrapper1");
				// YZQ ADD -- End
		...
		prev : function(){
			if(!busy){
				// YZQ ADD -- Start
				//if (opts.curr == 6) $("#bwrapperDiv").removeClass("bwrapper1").addClass("bwrapper2");
				//else if (opts.curr == 2) $("#bwrapperDiv").removeClass("bwrapper2").addClass("bwrapper1");
				// YZQ ADD -- End
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//add overlays
	if(opts.overlays){
		overlayP = $('<div class="b-overlay b-overlay-prev b-prev" title="上一页"></div>').appendTo(b);
		overlayN = $('<div class="b-overlay b-overlay-next b-next" title="下一页"></div>').appendTo(b);
	...
	//add tabs
	if(opts.tabs){
		tabP = $('<div class="b-tab b-tab-prev b-prev" title="上一页">上一页</div>').appendTo(b);
		tabN = $('<div class="b-tab b-tab-next b-next" title="下一页">下一页</div>').appendTo(b);
		tabs = b.find('.b-tab');
		...
		if(opts.direction == 'RTL'){
			tabN.html('Previous').attr('title','上一页');
			tabP.html('Next').attr('title','下一页');
		}
	}else{
		b.css({'marginTop': 0});
	}
	...
	//add arrows
	if(opts.arrows){
		arrowP = $('<div class="b-arrow b-arrow-prev b-prev" title="上一页"><div>上一页</div></div>').appendTo(b);
		arrowN = $('<div class="b-arrow b-arrow-next b-next" title="下一页"><div>Next</div></div>').appendTo(b);
		arrows = b.find('.b-arrow');
		
		//update ctrls for RTL direction
		if(opts.direction == 'RTL'){
			arrowN.html('<div>Previous</div>').attr('title','上一页');
			arrowP.html('<div>Next</div>').attr('title','下一页');
		}
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function initPages(){		
		...
		if(opts.closed){
			$(empty).attr({'title':opts.closedFrontTitle || "起始页", 'rel':opts.closedFrontChapter || "书本起始页"}).prependTo(src);
			src.children().last().attr({'title':opts.closedBackTitle || "结束页", 'rel':opts.closedBackChapter || "书本结束页"});		
			src.append(empty);		
		}
...
$.fn.booklet.defaults = {
	...
	hovers:             true,                            // enables preview pageturn hover animation, shows a small preview of previous or 下一页 on hover
	...
	next:               null,                            // selector for element to use as click trigger for 下一页
	prev:               null,                            // selector for element to use as click trigger for 上一页
