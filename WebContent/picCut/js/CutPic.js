...
//全局变量定义
var SCALE_SIZE = 150;
var CANVAS_WIDTH = 284*2; //画布的宽 568
var CANVAS_HEIGHT = 200*2; //画布的高 400
var ICON_SIZE_W = 136*2;  //截取框的大小，272
var ICON_SIZE_H = 170*2;  // 340
var LEFT_EDGE = (CANVAS_WIDTH - ICON_SIZE_W) / 2; //148
var TOP_EDGE = (CANVAS_HEIGHT - ICON_SIZE_H) / 2; //30
...
$(window).load(function() {
    ...
    if(realWidth>3000||realHeight>3000){
    	SCALE_SIZE=200;
    }
    if(realWidth<300||realHeight<300){
    	SCALE_SIZE=120;
    }
    ...
    //计算缩放比例开始
    var min_ICON_SIZE = Math.min(ICON_SIZE_H,ICON_SIZE_W);
    var minFactor = min_ICON_SIZE / Math.max(realWidth, realHeight);
    if (ICON_SIZE_W > realWidth && ICON_SIZE_H > realHeight) {
        minFactor = 1;
    }
    var factor = minFactor > 0.25 ? 16.0 : 8.0 / Math.sqrt(minFactor);
    ...
    }else if (realWidth < CANVAS_WIDTH && realHeight < CANVAS_HEIGHT){
    	if (realWidth / CANVAS_WIDTH > realHeight / CANVAS_HEIGHT) {
            scaleFactor = ICON_SIZE_H / realHeight;
        }
        else {
            scaleFactor = ICON_SIZE_W / realWidth;
        }
    }
    //计算缩放比例结束
    ...
    $(".child").css("left", SCALE_SIZE * (Math.log(scaleFactor * factor) / Math.log(factor)) + "px");
    ...
    $(".child").draggable(
  {
      ...
      var left = parseInt($(this).css("left"));
          ...
          scaleFactor = Math.pow(factor, (left / SCALE_SIZE - 1));
    ...
    var scrollFunc = function(e, delta) {
