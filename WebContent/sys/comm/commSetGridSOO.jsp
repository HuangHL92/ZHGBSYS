...
String pageSize = "20";
try{
	if(session.getAttribute("SOOPageSize") != null && !session.getAttribute("SOOPageSize").equals("")){
		 pageSize = session.getAttribute("SOOPageSize").toString(); 				//判断是否设置了自定义每页数量，如果设置了使用自定义
	}	
}catch(Exception e){
...
<script type="text/javascript">
	var maxPageSize = 500;//所设页数大于最大允许页数200条
	function doSetPageSize(){
		...
		if(pagesize==null||pagesize==""){
			...
			window.location="<%=request.getContextPath()%>/PublishFileServlet?method=SysOrgOtherPageSize&pageSize="+pagesize;
