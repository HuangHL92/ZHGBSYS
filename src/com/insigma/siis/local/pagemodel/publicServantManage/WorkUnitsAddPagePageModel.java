 ...
 *
 */
public class WorkUnitsAddPagePageModel extends PageModel {	
...
/*			Long a0194 = a01.getA0194();
			...
			if(a01.getA0188() == null || a01.getA0188().equals("")){		//乡镇党政正职经历为空
				this.getExecuteSG().addExecuteCode("document.getElementById('a0188').checked=false;");
			}
			
			if(a01.getA0132() == null || a01.getA0132().equals("")){		//乡镇（街道）党（工）委书记
				this.getExecuteSG().addExecuteCode("document.getElementById('a0132').checked=false;");
			}
			if(a01.getA0133() == null || a01.getA0133().equals("")){		//乡镇（街道）镇长（主任）
				this.getExecuteSG().addExecuteCode("document.getElementById('a0133').checked=false;");
			}
			
			if(a01.getFkbs()!=null && a01.getFkbs().equals("1") && 
					a01.getFkly()!=null && a01.getFkly().equals("2")) {
				this.getExecuteSG().addExecuteCode("$h.setDisabled($h.disabledButtons.a02);");
			}
			
			//分管工作查A86
//			CommonQueryBS cq=new CommonQueryBS();
//			cq.getMapBySQL("select");
			...
			//根据贵州客户要求，进入工作单位及职务，判断当前人员是否存在已免的主职务。如果存在，进行提示
			List<Object[]> list = (List<Object[]>)sess.createSQLQuery("select a0201a,a0215a from a02 where a02.a0279 = '1' and a02.a0255 = '0' and a02.a0000 = '"+a0000+"'").list();
			if(list!=null&&list.size()>0){
				Object[] objs = list.get(0);
				String a0201a = ""+objs[0];
				String a0215a = ""+objs[1];
				this.setMainMessage("本人的"+a0201a+a0215a+"优先排序已免，请适时修改。");
			}
//			//查询主职务条数
			String sql="select * from a02 where a0000='"+a0000+"' and A0279=1";
			ArrayList<Object> a2019List=(ArrayList<Object>) sess.createSQLQuery(sql).list();
			if(a2019List!=null&&a2019List.size()>1) {
				this.setMainMessage("此人有多条优先排序");
			}else if(a2019List==null||a2019List.size()<1){
				this.setMainMessage("此人没有优先排序");
			}
...
/*		this.getExecuteSG().addExecuteCode("setA0201eDisabled();");*/
	...
	public int copy()throws RadowException{
		...
		a02.setA0200(UUID.randomUUID().toString());
		HBSession sess = HBUtil.getHBSession();
		sess.save(a02);	
		this.setNextEventName("WorkUnitsGrid.dogridquery");//工作单位及职务列表
		return EventRtnType.NORMAL_SUCCESS;
...
//		}
		...
		if(a0279 != null && a0279.equals("1")){				//如果是主职务，则将其他职务a0279更新为0
			...
			HBUtil.executeUpdate("update a02 set a0279 = '"+0+"' where a0000 = '"+a0000+"' and a0200 !='"+a02.getA0200()+"'");
			sess.flush();
		}
...
//		}
		...
		//职务名称a0215a
		/*
		 * if(a0215a.length() > 50){ this.setMainMessage("职务名称不能超过50字！"); return
		...
		try {
			...
			if(a0200==null||"".equals(a0200)){
				...
				String a0283g = a02.getA0283g();
				if(!StringUtil.isEmpty(a0283g)) {
					a0283g=a0283g.replace(",", "，");
					if(a0283g.startsWith("，")) {
						a0283g = a0283g.substring(1);
					}
					if(a0283g.endsWith("，")) {
						a0283g=a0283g.substring(0, a0283g.length()-1);
					}
					a02.setA0283g(a0283g);
				}
				  //常规名册输出职务
				a02_old.setA0283g(a02.getA0283g());
				sess.update(a02_old);	
				//sess.createSQLQuery("update a02 a set a.a0283g =  where a.")
				//sess.flush();
			}
			...
			//更新职务层次和职级信息
			/*boolean gxzw = false;
			if(gwcode!=null && !gwcode.equals("")) {
				List<Jggwconf> confs = sess.createQuery(" from Jggwconf where gwcode='"+gwcode+"' and b0111='"+a02.getA0201b()+"'").list();
				if(confs.size() > 0) {
					Jggwconf cnf = confs.get(0);
					String zwcode = cnf.getZwcode();
					String zjcode = cnf.getZwcode();
					if(zwcode!=null && !zwcode.equals("")) {
						List<A05> a05s = sess.createQuery(" from A05 where a0531='0' and a0000='"+a0000+"' and a0524='1'").list();
						if(a05s.size() > 0) {
							A05 a05 = a05s.get(0);
							if(!a05.getA0501b().equals(zwcode)) {
								...
								sess.flush();
								A05 a05new = new A05();
								a05new.setA0000(a0000);
								a05new.setA0501b(zwcode);
								a05new.setA0504(date);
								a05new.setA0524("1");
								a05new.setA0525("1");
								a05new.setA0531("0");
								sess.save(a05new);
								String a0501bName = HBUtil.getValueFromTab("CODE_NAME", "CODE_VALUE", " code_type='ZB09' and code_value = '"+zwcode+"'");
								this.getExecuteSG().addExecuteCode("realParent.setA0221Value('"+(zwcode==null?"":zwcode)+"','"+a0501bName+"')");//页面设置 人物信息表的现职务层次
								this.getExecuteSG().addExecuteCode("realParent.setA0288Value('"+(date==null?"":date)+"')");//页面设置 人物信息表的现职务层次时间
								a01.setA0221(zwcode); //数据库设置 人物信息表的现职务层次 当前被选的职务层次
								a01.setA0288(date);
							}
						}
					}
			...
			System.out.println("a0192a======>"+a0192a);
			a01.setA0192a(a0192a);
			a01.setA0192(a0192a);//根据客户要求，简称存储与全称一致
			a01.setA0197(a0197);
			a01.setA0188(a0188);
			a01.setA0132(a0132);
			a01.setA0133(a0133);
			...
			a0192a = a01.getA0192a() == null ? "" : a01.getA0192a();
			a0192 = a01.getA0192a() == null ? "" : a01.getA0192a();
			this.getExecuteSG().addExecuteCode("WorkflushWindowsOpen('" + a0192 + "','" + a0192a + "')");
...
*/					
			...
			List<Jggwconf> confs = sess.createQuery(" from Jggwconf where gwcode='"+gwcode+"' and b0111='"+a02.getA0201b()+"'").list();
			List<A05> a05s = sess.createQuery(" from A05 where a0531='0' and a0000='"+a0000+"' and a0524='1'").list();
			boolean updatezwxx = false;
			if(confs.size()>0 && a05s.size()>0) {
				String a05cc = a05s.get(0).getA0501b();
				String cfgcc = confs.get(0).getZwcode();
				if(cfgcc!=null && !cfgcc.equals("")) {
					if(a05cc!=null && !a05cc.equals("")) {
						if(!a05cc.equals(cfgcc)) {
							String minsql= "select code_value from code_value t where t.code_type='ZB09' "
									+ "and t.code_value in('"+a05cc+"','"+cfgcc+"') order by code_value ";
							List<Object> ml = sess.createSQLQuery(minsql).list();
							String mcode = ml.get(0).toString();
							if(mcode.equals(cfgcc)) {
								updatezwxx = true;
							}
						}
					} else {
			...
			if(updatezwxx){
				this.getExecuteSG().addExecuteCode("$h.confirm('系统提示','此职务对应职务层次高于现职务层次，是否更新?',350,function(id){" +
						"if(id=='ok'){" +
							"radow.doEvent('UpdateGwxxBtn.onclick','"+updateJGZW+"');	" +
							"}else if(id=='cancel'){" +
							"	" +
							"}" +
						"});");
			...
			//根据要求，不需要后面自动生成全称，逻辑
//			else if(updateJGZW){
//				this.getExecuteSG().addExecuteCode("radow.doEvent('UpdateTitleBtn.onclick');");
//							//+ "updateZWMCMC();	");//提示是否更新职务名册名册
//			
//			}
			else{
	 ...
	 */
	@PageEvent("updateZWMCMC.onclick")
	@Transaction
	@NoRequiredValidate
	public int updateZWMCMC() throws RadowException {
		HBSession session = HBUtil.getHBSession();
		String a0200 = this.getPageElement("a0200").getValue();
		String sql = "select  b.A0215A || (select ',' || wm_concat(f.A0201A || f.A0215A)" + 
				" from (select c.*" + 
				" from a02 c" + 
				" where 1 = 1" + 
				" and c.a0255 = '1'" + 
				" and c.A0215A is not null" + 
				" order by a0223 asc, a0243 desc) f" + 
				" where b.a0000 = f.a0000" + 
				" and b.A0200 <> f.A0200) a0283g from a02 b where b.a0200 = '"+a0200+"'";
		Object uniqueResult = session.createSQLQuery(sql).uniqueResult();
		String a0283g = uniqueResult.toString();
		a0283g=a0283g.replace(",", "，");
		if(a0283g.startsWith("，")) {
			a0283g = a0283g.substring(1);
		}
		...
		this.getPageElement("a0283g").setValue(a0283g);
		session.createQuery("update A02 a set a.a0283g = '"+a0283g+"' where a.a0200 = '"+a0200+"'").executeUpdate();
		session.flush();
		return EventRtnType.NORMAL_SUCCESS;
	}
	/**
	 ...
	 * @throws RadowException
	 */
	@PageEvent("UpdateGwxxBtn.onclick")
	@Transaction
	@NoRequiredValidate
	public int UpdateGwxxBtn(String boo) throws RadowException {
		HBSession sess = null;
		String gwcode = this.getPageElement("a0215aC").getValue();
		...
		try {
			...
			//更新职务层次和职级信息
			if(gwcode!=null && !gwcode.equals("")) {
				List<Jggwconf> confs = sess.createQuery(" from Jggwconf where gwcode='"+gwcode+"' and b0111='"+b0111+"'").list();
				if(confs.size() > 0) {
					Jggwconf cnf = confs.get(0);
					String zwcode = cnf.getZwcode();
					String zjcode = cnf.getZwcode();
...
/** *********************************************工作单位及职务(a02)******************************************************************** */
	...
	public int workUnitsGridOnRowClick() throws RadowException{ 
		...
		try {
			...
			//System.out.println("1121212------------------");
			//从A86中查询A0229
			//com.insigma.siis.local.pagemodel.comm.CommonQueryBS cq = new com.insigma.siis.local.pagemodel.comm.CommonQueryBS();
			//System.out.println("1121212------------------");
			//List<HashMap> a0229maps = CommonQueryBS.getQueryInfoByManulSQL("select a8600,a0299 from A86 where A8602='1' and A0200='" + a0200 + "'");
			/*HashMap<String, Object> a0229map = cq.getMapBySQL();*/
			//System.out.println("1121212------------------");
//			if (a0229maps != null && a0229maps.size() > 0) {
//				HashMap a0229map = a0229maps.get(0);
//				String a0229 = a0229map.get("a0299") == null ? "" : a0229map.get("a0299").toString();
//				String a8600 = a0229map.get("a8600") == null ? "" : a0229map.get("a8600").toString();
//				a02.setA0229(a0229);
//				this.getPageElement("a8600").setValue(a8600);
//			} else {
//				a02.setA0229("");
//			}
			//System.out.println("1121212------------------");
			...
			*/
			...
			if(a02.getA0248() == null || !a02.getA0248().equals("1")){		//不占职数
				this.getExecuteSG().addExecuteCode("document.getElementById('a0248').checked=false;");
			}
			if(a02.getA0222b() == null || !a02.getA0222b().equals("1")){		//保留职级
				this.getExecuteSG().addExecuteCode("document.getElementById('a0222b').checked=false;");
			}
			if(a02.getA0277() == null || !a02.getA0277().equals("1")){		//是否超配
				this.getExecuteSG().addExecuteCode("document.getElementById('a0277').checked=false;");
			}
			if(a01.getFkbs()!=null && a01.getFkbs().equals("1") && 
					a01.getFkly()!=null && a01.getFkly().equals("2")) {
	...
	public int UpdateTitleBtn(String id) throws RadowException {
		...
		//根据贵州客户要求，如果职务名称为空，则不参与生成全称、简称的操作
		String sql = "from A02 where a0000='"+a0000+"' and a0215a is not null and a0281='true' order by a0223";//-1 其它单位and a0201b!='-1'
		...
		if(list!=null&&list.size()>0){
			...
			//this.getExecuteSG().addExecuteCode("window.parent.document.getElementById('tab_baseInfo').contentWindow.document.getElementById('a0192b').value='"+a01.getA0192b()+"';");
			if(isEvent) {
				String a0192a = a01.getA0192a() == null ? "" : a01.getA0192a();
				String a0192 = a01.getA0192a() == null ? "" : a01.getA0192a();
				this.getExecuteSG().addExecuteCode("WorkflushWindowsOpen('" + a0192 + "','" + a0192a + "')");
	...
	public int UpdateTimeBtn(String id) throws RadowException {
		...
		if(list!=null&&list.size()>0){
			...
			List<String> List= HBUtil.getHBSession().createSQLQuery("select a0243 from a02 where a0000='"+a0000+"' and a0281='true' order by substr(a0243,0,6) desc").list();
			String rzsj="";
			if(List.size()>0) {
				rzsj=List.get(0);
			}
...
//					"window.realParent.ajaxSubmit('genResume');");
	...
	public int setZB08Code(String id)throws RadowException{
		...
		if(B0194 != null && B0194.equals("2")){								//任职机构如果选择了内设机构，则任职状态和职务名称不可选
			...
			this.getExecuteSG().addExecuteCode(""
					+ "document.getElementById('a0201e_combo').disabled=true;document.getElementById('a0201e_combo').style.backgroundColor='#EBEBE4';"
					+ "document.getElementById('a0201e_combo').style.backgroundImage='none';Ext.getCmp('a0201e_combo').setDisabled(true);"
	...
	//判断统计机构关系是否合理
	@PageEvent("a0215aCChange")
	@NoRequiredValidate
	public int a0215aCChange() throws RadowException{
		HBSession sess = null;
		String gwcode = this.getPageElement("a0215aC").getValue();
		String a0215aC_combo = this.getPageElement("a0215aC_combo").getValue();
		String b0111 = this.getPageElement("a0201b").getValue();
		//String a0000 = this.getPageElement("a0000").getValue();
		try {
			sess = HBUtil.getHBSession();
			this.getPageElement("a0215a").setValue(a0215aC_combo);
			if(gwcode!=null && !gwcode.equals("")) {
				List<Jggwconf> confs = sess.createQuery(" from Jggwconf where gwcode='"+gwcode+"' and b0111='"+b0111+"'").list();
				if(confs.size() > 0) {
					Jggwconf cnf = confs.get(0);
					String a0219 = cnf.getA0219();
					String a0201d = cnf.getA0201d();
					String a0201e = cnf.getA0201e();
					if(a0219!=null && a0219.equals("1")) {
						this.getPageElement("a0219").setValue("1");
					} else {
						this.getPageElement("a0219").setValue("0");
					}
					if(a0201d!=null && a0201d.equals("1")) {
						this.getPageElement("a0201d").setValue("1");
						this.getExecuteSG().addExecuteCode("setA0201eDisabled();");
						this.getPageElement("a0201e").setValue(a0201e);
					} else {
						this.getPageElement("a0201d").setValue("0");
						this.getExecuteSG().addExecuteCode("setA0201eDisabled();");
					}
