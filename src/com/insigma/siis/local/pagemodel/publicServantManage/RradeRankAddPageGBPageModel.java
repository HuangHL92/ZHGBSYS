...
public class RradeRankAddPageGBPageModel extends PageModel {
			...
			if (a0500 == null || "".equals(a0500)) {
				if("1".equals(a0524)){//状态为在任时，进行判断，若之前有在任的则提示
					...
					if(list_a05.size()>0){//检测到之前有状态为在任的
						...
						if(a0501b!=null){
							a0501bName = HBUtil.getValueFromTab("CODE_NAME", "CODE_VALUE", " code_type='ZB148' and code_value = '"+a0501b+"'");
...
//						this.getExecuteSG().addExecuteCode("realParent.setA0192cValue('')");//页面设置 人物信息表的现职务时间
				...
				applog.createLogNew("3A05","现职级修改","现职级",a0000,a01.getA0101(), new Map2Temp().getLogInfo(a05_old,a05));
