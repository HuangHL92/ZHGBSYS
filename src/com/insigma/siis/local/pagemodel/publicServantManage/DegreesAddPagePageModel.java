 ...
 *
 */
public class DegreesAddPagePageModel extends PageModel {	
...
//				//获取用户输入的学历学位信息
//				String a0824_new = a08.getA0824();//获取所学专业名称
//				if(a0824_new.contains("专业")) {
//					a0824_new=a0824_new.replace("专业", "");//替换掉原有的数据
//					a08.setA0824(a0824_new);
//				}
		a08.setA0000(a0000);
		HBSession sess = null;
		try {
			...
			if(isSame){
				...
				this.getExecuteSG().addExecuteCode("openSelectWin('"+a0000+"')");
 ...
 * @throws AppException *********************************************************************/
	...
	private void updateZGXueliXuewei(String a0000, HBSession sess) throws AppException {
		...
		sess.flush();
		String sql="select a0800,a0801b,case when substr(a0801b,1,1)=1 then '1' when substr(a0801b,1,1)=2 and a0801b in ('21','23','24','2A','2B','2E') then '2' "
				+ " when substr(a0801b,1,1)=2 and a0801b in ('22','2C','2D','2F') then '3' else '4' end ||a0801b as a0801b_2 from a08 where a0000='"+a0000+"' and a0837='1' and a0831='1' and a0899='true'";
		String sql2="select a0800,a0801b,case when substr(a0801b,1,1)=1 then '1' when substr(a0801b,1,1)=2 and a0801b in ('21','23','24','2A','2B','2E') then '2' "
				+ " when substr(a0801b,1,1)=2 and a0801b in ('22','2C','2D','2F') then '3' else '4' end ||a0801b as a0801b_2 from a08 where a0000='"+a0000+"' and a0837='2' and a0838='1' and a0899='true'";
...
//			String zzxl=zzxueli.substring(0, 1);
			String qrzxl=qrz[2].toString();
			String zzxl=zz[2].toString();
...
/*		if(xl!=null){
	...
	private void setxuexiaoxinxi(String a0000) {
		HBSession sess=HBUtil.getHBSession();
		A01 a01= (A01)sess.get(A01.class, a0000);
		
		refreshRMBxlxwxx(a01);
		a01= (A01)sess.get(A01.class, a0000);
		
		//在职的  双学位情况下,在职学位的学校如果包含学历的学校则去掉包含的,其他直接保存  双学历情况也是如此
		/*String zzxlxx=a01.getZzxlxx();
		String zzxwxx=a01.getZzxwxx();
		if(zzxwxx!=null&&!"".equals(zzxwxx)) {
			if(zzxlxx!=null&&(zzxwxx.contains(zzxlxx)&&!zzxwxx.equals(zzxlxx))) {
				zzxwxx=zzxwxx.replace(zzxlxx, "").replace(",", "");
				a01.setZzxwxx(zzxwxx);
			}else {
				a01.setZzxwxx(zzxwxx);
			}
		}
		
		if(zzxlxx!=null&&!"".equals(zzxlxx)) {
			if(zzxwxx!=null&&(zzxlxx.contains(zzxwxx)&&!zzxwxx.equals(zzxlxx))) {
				zzxlxx=zzxlxx.replace(zzxwxx, "").replace(",", "");
				a01.setZzxlxx(zzxlxx);
			}else {
				a01.setZzxlxx(zzxlxx);
			}
		}
		...
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('zzxl').value='"+(a01.getZzxl()==null?"":a01.getZzxl())+"'");
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('zzxlxx').value='"+(a01.getZzxlxx()==null?"":a01.getZzxlxx())+"'");
		//人员界面的P标签也要改 yzk 2019.12.12
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('zzxlxx_p').innerHTML='"+(StringUtil.isEmpty(a01.getZzxlxx())?(a01.getZzxwxx()==null?"":a01.getZzxwxx()):a01.getZzxlxx())+"';");
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('zzxlxx_p').title='"+(StringUtil.isEmpty(a01.getZzxlxx())?(a01.getZzxwxx()==null?"":a01.getZzxwxx()):a01.getZzxlxx())+"'");
	
		//人员基本信息界面
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('qrzxl').value='"+(a01.getQrzxl()==null?"":a01.getQrzxl())+"'");
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('qrzxlxx').value='"+(a01.getQrzxlxx()==null?"":a01.getQrzxlxx())+"'");
		//人员界面的P标签也要改 yzk 2019.12.12
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('qrzxlxx_p').innerHTML='"+(StringUtil.isEmpty(a01.getQrzxlxx())?(a01.getQrzxwxx()==null?"":a01.getQrzxwxx()):a01.getQrzxlxx())+"'");
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('qrzxlxx_p').title='"+(StringUtil.isEmpty(a01.getQrzxlxx())?(a01.getQrzxwxx()==null?"":a01.getQrzxwxx()):a01.getQrzxlxx())+"'");
		...
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('qrzxwxx').value='"+(a01.getQrzxwxx()==null?"":a01.getQrzxwxx())+"'");
		//人员界面的P标签也要改 yzk 2019.12.12
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('qrzxwxx_p').innerHTML='"+(a01.getQrzxwxx()==null?"":a01.getQrzxwxx())+"'");
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('qrzxwxx_p').title='"+(a01.getQrzxwxx()==null?"":a01.getQrzxwxx())+"'");
		...
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('zzxwxx').value='"+(a01.getZzxwxx()==null?"":a01.getZzxwxx())+"'");
		//人员界面的P标签也要改 yzk 2019.12.12
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('zzxwxx_p').innerHTML='"+(a01.getZzxwxx()==null?"":a01.getZzxwxx())+"'");
		this.getExecuteSG().addExecuteCode("window.realParent.document.getElementById('zzxwxx_p').title='"+(a01.getZzxwxx()==null?"":a01.getZzxwxx())+"'");
	...
	private void checkXL(HBSession sess,String a0837,String a0000) throws AppException {//a0837教育类别
		...
		sql1+="order by a0838 desc";
		List<A08> list1 = sess.createSQLQuery(sql1).addEntity(A08.class).list();
		if(list1!=null&&list1.size()>0){
			A08 xueli = list1.get(0);
			String xldm = xueli.getA0801b();//院系
			String yx_xl = xueli.getA0814();//院系
			String zy_xl = xueli.getA0824();//专业
			if(yx_xl==null){
				yx_xl = "";
...
//				zy_xl += "专业";
			...
			String xlqm=xueli.getA0801a();
			if(xlqm.contains("硕士研究生") || xlqm.contains("博士研究生")) {
				xl="研究生";
			}else{
				xl=xlqm;
			}
//			xl = xueli.getA0801a();//学历 中文
			...
			String a0108b = xldm.substring(0,1);
			String sql2 = "select * from a08 where a0000='"+a0000+"' and a0837='"+a0837+"' and a0899='true' and a0801b = '"+xldm+"' ";  //以学历编码升序排列
			if("1".equals(a0837)){
				sql2+=" and a0831<>'1'";
			}
			if("2".equals(a0837)){
			...
			List<A08> list2 = sess.createSQLQuery(sql2).addEntity(A08.class).list();
			if(list2.size()>0){
					A08 xueli2=list2.get(0);
					String[] xlInfo2 = mergeXLXX(xueli2, xl, xlxx, yx_xl);
					xl = xl + xlInfo2[0];
					xlxx = xlxx + xlInfo2[1];
					if(list2.size()==2){
						A08 xueli3=list2.get(1);
						String[] xlInfo3 = mergeXLXX(xueli3, xl, xlxx, yx_xl);
						if(!xlInfo2[0].equals(xlInfo3[0])){
							xl = xl + xlInfo3[0];
						}
						if(!xlInfo2[1].equals(xlInfo3[1])){
							xlxx = xlxx + xlInfo3[1];
						}
					}
			}
	...
	private String[] mergeXLXX(A08 xueli2,String xl,String xlxx,String yx_xl){
		String xlr="";
		String xlxxr="";
		String xl2qm=xueli2.getA0801a();
		String xl2="";
		if(xl2qm.contains("硕士研究生") || xl2qm.contains("博士研究生")) {
			xl2="研究生";
		}else{
			xl2=xl2qm;
		}
		String yx_xl2=xueli2.getA0814();
		String zy_xl2=xueli2.getA0824();
		if(yx_xl2==null){
			yx_xl2 = "";
		}
		if(zy_xl2==null){
		...
		if(xl2!=null&&!"".equals(xl2)){
			if(xl==null||"".equals(xl)){
				...
				if(StringUtil.isEmpty(xlxx)){
					xlxxr= yx_xl2+zy_xl2;
				}else{
					if (!"".equals(yx_xl2) && yx_xl2.indexOf("专业")<0 && yx_xl2.equals(yx_xl)) {
						//邹志林 2019-09-26 修改 如果是相同院校毕业则合并
						//xlxx=xlxx+"、"+zy_xl2;
						xlxxr = "、"+zy_xl2;
					} else {
						//xlxx=xlxx+","+yx_xl2+zy_xl2;
						xlxxr = "," + yx_xl2 + zy_xl2;
					}
				}
	...
	private void checkXW(HBSession sess,String a0837,String a0000) throws AppException{
		...
		sql+="order by a0838 desc";
		List<A08> list = sess.createSQLQuery(sql).addEntity(A08.class).list();
		if(list!=null&&list.size()>0){
			for(int i=0;i<list.size();i++) {
				A08 a08= list.get(i);
				xwb=a08.getA0901a();
				xwxxb=a08.getA0814()+a08.getA0824();
				String[] xw_s=xw.split(",");
				int xcy=-1;
				if(xw_s.length>0) {
				for(int j=0;j<xw_s.length;j++) {
					if(xwb.equals(xw_s[j])) {
							xcy=1;
						}
					}
				}
				if(xcy==-1) {
					...
					xw=xw;
					String[] xwxx_s=xwxx.split(",");
					if(xwxx_s.length>0) {
						for(int k=0;k<xwxx_s.length;k++) {
							if(k==0) {
								xwxx=xwxx_s[k]+","+xwxxb;
							}else {
								xwxx=xwxx+","+xwxx_s[k];
...
//							xwxx= yx_xw2+zy_xw2;
//						}else{
//							if (!"".equals(yx_xw2) && yx_xw2.indexOf("专业")<0 && yx_xw2.equals(yx_xw)) {
//								//邹志林 2019-09-26 修改 如果是相同院校毕业则合并
//								xwxx=xwxx+"、"+zy_xw2; 
//							}else {
...
//			}
	 ...
	 * @param a01
	 */
	private void refreshRMBxlxwxx(A01 a01) {
		//============================邹志林 2019-09-26 ====================
		String qrzxlxx = a01.getQrzxlxx();
		String qrzxwxx = a01.getQrzxwxx();
		qrzxlxx = qrzxlxx==null?"":qrzxlxx;
		qrzxwxx = qrzxwxx==null?"":qrzxwxx;
		
		
		//学历学校判断 zoul2021年8月6日15:42:32
		String[] qrzxlxxs = qrzxlxx.split(",|，");
		String[] qrzxwxxs = qrzxwxx.split(",|，");
		StringBuilder xwxxsb = new StringBuilder();
		for(String xw : qrzxwxxs){
			boolean flag = true;
			for(String xl : qrzxlxxs){
				if(xl.equals(xw)){
					flag = false;
					break;
				}
		...
		if(xwxxsb.length()>0){
			xwxxsb.deleteCharAt(xwxxsb.length()-1);
		}
		a01.setQrzxwxx(xwxxsb.toString());
		qrzxwxx = a01.getQrzxwxx();
		//学历学校判断
		String zzxlxx = a01.getZzxlxx()==null?"":a01.getZzxlxx();
		String zzxwxx = a01.getZzxwxx()==null?"":a01.getZzxwxx();
		String[] zzxlxxs = zzxlxx.split(",|，");
		String[] zzxwxxs = zzxwxx.split(",|，");
		StringBuilder zzxwxxsb = new StringBuilder();
		for(String xw : zzxwxxs){
			boolean flag = true;
			for(String xl : zzxlxxs){
				if(xl.equals(xw)){
					flag = false;
					break;
				}
		...
		if(zzxwxxsb.length()>0){
			zzxwxxsb.deleteCharAt(zzxwxxsb.length()-1);
		}
		a01.setZzxwxx(zzxwxxsb.toString());
		zzxwxx = a01.getZzxwxx();
		
		
		HBUtil.getHBSession().update(a01);
		
		
		if ("".equals(qrzxlxx) || "".equals(qrzxwxx) || qrzxlxx.equals(qrzxwxx)) {
			//System.out.println("qrzxlxxTD===>rowspan:2  qrzxwxxTD===>display:none");
			this.getExecuteSG().addExecuteCode("window.realParent.$('#qrzxlxxTD').attr('rowspan',2);");
			this.getExecuteSG().addExecuteCode("window.realParent.$('#qrzxwxxTD').css('display','none');");
		}else {
			//System.out.println("qrzxlxxTD===>rowspan:1  qrzxwxxTD===>block");
			this.getExecuteSG().addExecuteCode("window.realParent.$('#qrzxlxxTD').attr('rowspan',1);");
			this.getExecuteSG().addExecuteCode("window.realParent.$('#qrzxwxxTD').css('display','');");
		}
		
		
		if ("".equals(zzxlxx) || "".equals(zzxwxx) || zzxlxx.equals(zzxwxx)) {
			//System.out.println("zzxlxxTD===>rowspan:2  zzxwxxTD===>display:none");
			this.getExecuteSG().addExecuteCode("window.realParent.$('#zzxlxxTD').attr('rowspan',2);");
			this.getExecuteSG().addExecuteCode("window.realParent.$('#zzxwxxTD').css('display','none');");
		}else {
			//System.out.println("zzxlxxTD===>rowspan:1  zzxwxxTD===>block");
			this.getExecuteSG().addExecuteCode("window.realParent.$('#zzxlxxTD').attr('rowspan',1);");
			this.getExecuteSG().addExecuteCode("window.realParent.$('#zzxwxxTD').css('display','');");
		}	
		//=====================================================
	}
...
/*********************************************************************************************************************************/	
	...
	public int degreesgridQuery(int start,int limit) throws RadowException{
		...
		String a0000 = this.getPageElement("subWinIdBussessId").getValue();
//		String a0000 = this.getRadow_parent_data();
		String sql = "select * from A08 where a0000='"+a0000+"' order by a0837,case when substr(a0801b,1,1)=1 then '1' when substr(a0801b,1,1)=2 and a0801b in ('21','23','24','2A','2B','2E') then '2' "
				+ " when substr(a0801b,1,1)=2 and a0801b in ('22','2C','2D','2F') then '3' else '4' end ||a0801b ";
...
//		String a0000 = this.getRadow_parent_data();
	...
	public int deleteRow(String a0800)throws RadowException, AppException{
		...
		try {
			...
			//applog.createLog("3083", "A08", a01.getA0000(), a01.getA0101(), "删除记录", new ArrayList<Object[]>());
			
			//显示删除了哪条
			A08 delete = new A08();
			delete.setA0834(a08.getA0834());
			delete.setA0835(a08.getA0835());
			delete.setA0832(a08.getA0832());
			delete.setA0831(a08.getA0831());
			applog.createLog("3083", "A08", a01.getA0000(), a01.getA0101(), "删除记录", new Map2Temp().getLogInfo(a08, delete));
			...
			if(checked){
				...
				if(isSame){
					//this.setRadow_parent_data(a08.getA0000());
					this.getExecuteSG().addExecuteCode("openSelectWin('"+a08.getA0000()+"')");
					a08 = new A08();
					PMPropertyCopyUtil.copyObjValueToElement(a08, this);
					return EventRtnType.NORMAL_SUCCESS;
				}else{
	...
	public int printout(String a0000,HBSession sess,String a0837) throws RadowException{
		...
		sess.createSQLQuery(sql).executeUpdate();
		sess.flush();
		String sql1 = "select a0800,a0801b,case when substr(a0801b,1,1)=1 then '1' when substr(a0801b,1,1)=2 and a0801b in ('21','23','24','2A','2B','2E') then '2' "
				+ " when substr(a0801b,1,1)=2 and a0801b in ('22','2C','2D','2F') then '3' else '4' end ||a0801b as a0801b_2 from a08 where a0000='"+a0000+"' and a0837='"+a0837+"'";// 输出的最高在职学历 
		List<Object[]> list1 = sess.createSQLQuery(sql1).list();
		if(list1!=null&&list1.size()>0){
			Collections.sort(list1,new Comparator<Object[]>(){//学历排序
				@Override
				public int compare(Object[] o1, Object[] o2) {
					...
					//return sortmap.get(a0801b_1).compareTo(sortmap.get(a0801b_2));
					String a0801b_3 = o1[2]==null?"":o1[2].toString();//学历代码
					String a0801b_4 = o2[2]==null?"":o2[2].toString();
					return a0801b_3.compareTo(a0801b_4);
				}
	...
	/*	if(list1!=null&&list1.size()==1){
		...
		//如果有多条记录,第一条记录学历代码不为空就是最高学历,剩余学历代码如果与第一条一样也不处理
		String xuelidaima= null;
		if(list1!=null&&list1.size()>0){
			Object[] a08=list1.get(0);
			xuelidaima=a08[1]==null?"":a08[1].toString();
			if(!StringUtil.isEmpty(xuelidaima)){
		...
		if(list2!=null&&list2.size()>0){
			...
			if(xuelidaima!=null && !"".equals(xuelidaima)) {
				sess.createSQLQuery(" update a08 set a0899='true' where a0000='"+a0000+
						"' and a0837='"+a0837+"' and a0801b = '"+xuelidaima+"'").executeUpdate();
			}
		}
		sess.flush();
		return EventRtnType.NORMAL_SUCCESS;
	...
	public boolean isSame(String a0000,HBSession sess){
		String sql="select case when substr(a0801b,1,1)=1 then '1' when substr(a0801b,1,1)=2 and a0801b in ('21','23','24','2A','2B','2E') then '2' "
				+ " when substr(a0801b,1,1)=2 and a0801b in ('22','2C','2D','2F') then '3' else '4' end ||a0801b as a0801b from a08 where a0000='"+a0000+"' and a0837='1' and a0831='1' and a0899='true'";
		String sql2="select case when substr(a0801b,1,1)=1 then '1' when substr(a0801b,1,1)=2 and a0801b in ('21','23','24','2A','2B','2E') then '2' "
				+ " when substr(a0801b,1,1)=2 and a0801b in ('22','2C','2D','2F') then '3' else '4' end ||a0801b as a0801b from a08 where a0000='"+a0000+"' and a0837='2' and a0838='1' and a0899='true'";
