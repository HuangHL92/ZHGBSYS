...
public class ImpZzbHzbThreadNewCaNull implements Runnable {
    ...
    public ImpZzbHzbThreadNewCaNull(String filename, String uuid, String b0111new,String a0165, CurrentUser user,UserVO userVo,String fxz) {
        this.uuid = uuid;
    ...
    public ImpZzbHzbThreadNewCaNull(String filename, String uuid, String b0111new,String a0165, CurrentUser user,UserVO userVo,String fxz,String isContain) {
        this.uuid = uuid;
        this.filename = filename;
        this.b0111new = b0111new;
        this.a0165 = a0165;
	...
	public void run() {
		...
		try {
			...
			/*if(houzhui.equalsIgnoreCase("zip")){
				...
				unzip = AppConfig.HZB_PATH + "/temp/upload/" + uuid + "/" + uuid;										// 解压路径
			...
			if("1".equals(isContain)) {
				impdeptid = CreateSysOrgBS.selectB0111BySubId(detps.get(0).getB0111());
				deptid =  detps.get(0).getB0111();
			}
			...
			if (imprecords != null && imprecords.size() > 0) {
				...
				imprecord.setIscontain(isContain);
				sess.update(imprecord);
				appendFileContent(logfilename, "创建临时表:"+ DateUtil.getTime()+"\n");					//记录日志
				if("7z".equalsIgnoreCase(houzhui)||"zip".equalsIgnoreCase(houzhui)||"hzb".equalsIgnoreCase(houzhui)){
				...
				ImpZzbPart1Thread t1 = new ImpZzbPart1Thread(userVo, houzhui, imprecordid, uuid, from_file, B0111, deptid, impdeptid, logfilename, control,"1",fxz);
				ImpZzbPart1Thread t2 = new ImpZzbPart1Thread(userVo, houzhui, imprecordid, uuid, from_file, B0111, deptid, impdeptid, logfilename, control,"2",fxz);
