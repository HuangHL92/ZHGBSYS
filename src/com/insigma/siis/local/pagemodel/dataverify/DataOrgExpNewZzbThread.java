...
public class DataOrgExpNewZzbThread implements Runnable {
...
//	private String lsry;
    ...
    public DataOrgExpNewZzbThread(String uuid, CurrentUser user,String gjgs,String fxzry,String gzlbry
	...
	public void run() {
		...
		try {
			...
			if (tabimp == null || tabimp.equals("") || tabimp.equals("1")) {
				...
				/*List list = sess.createSQLQuery("SELECT v_time FROM v_version order by v_seq desc").list();
				if(list != null && list.size() >0) {
					map.put("dataversion", list.get(0).toString());
				}else {
					map.put("dataversion", "20180101");
				}*/
...
//						//------------------------------------------------
						...
						DataOrgExpPartThread t1 = new DataOrgExpPartThread(uuid, gjgs, searchDeptid, zdcjg, userVo, newTable, b01, control, logfilename, "1",sign);
						DataOrgExpPartThread t2 = new DataOrgExpPartThread(uuid, gjgs, searchDeptid, zdcjg, userVo, newTable, b01, control,logfilename, "2",sign);
						control.setPath(path);
						control.setZippath(zippath);
						control.setThd1(new Thread(t1, "DataOrgExpPart_1_" + uuid));
