...
public class ImpHzbThreadZzb implements Runnable {
	...
	private String fxz;
    public ImpHzbThreadZzb(String filename, String uuid, CurrentUser user,UserVO userVo,String a0165,String fxz) {
        this.uuid = uuid;
	...
	public void run() {
		...
		UploadHzbFileBSZzb uploadbs = new UploadHzbFileBSZzb();		// 业务处理bs
		...
		try {
			...
			/*if("hzb".equalsIgnoreCase(houzhui)){*/
				...
				/*}
			if("zip".equalsIgnoreCase(houzhui)){
				unzip = AppConfig.HZB_PATH + "/temp/upload/" + uuid + "/";
				String zip = unzip + uuid;// 解压路径
			...
			if (imprecords != null && imprecords.size() > 0) {
				...
				/*int number2 = 26;*/																//未解析标的树木
				int number2 = 12;
				/*String[] tables = {"A01","A02","A06","A08","A11","A14","A15","A29","A30","A31","A36","A37","A41","A53","A57",
						"A60","A61","A62","A63","A64","B01" , "A05" , "A68", "A69", "A71", "A99Z1"};*/
				String[] tables = {"A01","A02","A06","A08","A14","A15","A36","A57","B01","A05","A99Z1","A30"};
...
//						uploadbs.saveData_SaxHander2(houzhui.toLowerCase(), table[i], imprecordid, t_n, uuid, from_file, B0111, deptid, impdeptid);
					...
					for(int i = 0;i<tables.length;i++){
						...
						appendFileContent(logfilename, tables[i]+"数据导入"+ DateUtil.getTime()+"\n"+"neicun："+(Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()));
						KingbsconfigBS.saveImpDetail(process_run,"1","提取第"+(number1++)+"张表"+tables[i]+"数据， 剩余"+(number2--)+"张。",imprecordid);
//						uploadbs.saveData_SaxHander2(houzhui.toLowerCase(), tables[i], imprecordid, t_n, uuid, from_file, B0111, deptid, impdeptid);
