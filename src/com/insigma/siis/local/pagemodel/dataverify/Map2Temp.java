...
/**
    ...
    public static List<String[]> getLogInfo(Object oldObj, Object newObj)
        ...
        for (String key : newMap.keySet()) {
        	...
        	}else if(key.equals("COMBOXAREA_A0114")){
        		key = "A0114A";
        	}else if(key.equals("COMBOXAREA_A0111")){
        		key = "A0111A";
        	}else if("A1701".equals(key)){
				if(val1!=null){
					val1 = val1.trim().replaceAll("\r", "").replaceAll("\n", "");
				}
				if(val2!=null){
					val2 = val2.trim().replaceAll("\r", "").replaceAll("\n", "");
				}
			}
        	...
        	//日志完善后     codetypeMapping 不在配置表里的不做日志记录 2018年10月16日
        	//if(fieldName!=null){
        		if(!val2.equals(val1) && !(StringUtil.isEmpty(val2) && StringUtil.isEmpty(val1)) ){
        			
            		String[] arr = {key, val1, val2,key};
            		list.add(arr);
            	}
        	//}
    ...
    public static void addCgList(String val1,String val2,String key,List<String[]> cglist){
    	if(!val2.equals(val1) && !(StringUtil.isEmpty(val2) && StringUtil.isEmpty(val1)) ){
