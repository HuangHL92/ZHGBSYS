...
public class xrdxChoosePageModel extends PageModel {
	...
	public int queryFromData() throws RadowException{
		...
		if (selectByInputYnIdHidden != null && !"".equals(selectByInputYnIdHidden)) {
			//按照导入批次选择
			sql2="select  a01.a0000, a0101, a0104,(substr(a01.a0107,1,4)||'.'||substr(a01.a0107,5,2)) a0107,a0192a,a0163 from "+Zsrm.getZjkTableName("a01")+" where a0000 in (select a0000 from "+Zsrm.getZjkTableName("TPHJPERSON")+" where TPZZ02='"+selectByInputYnIdHidden+"' )";
...
//							+ " and a01.status!='4' ";
			//} else {
				...
				sql2="select distinct "+mult+" a01.a0000,a01.a0101,a01.a0104,(substr(a01.a0107,1,4)||'.'||substr(a01.a0107,5,2)) a0107,a01.a0192a,a01.a0163 from A01 a01,A02 a02  where a01.A0000=a02.A0000  and status <> '4' and a01.a0000  in (select a02.a0000 from a02 a02 where a02.a0281='true' "  +
...
//     			List<String> pernum= HBUtil.getHBSession().createSQLQuery("select a0000 from edu_xrdx_ry where a0000='"+hm.get("a0000")+"'  and xrdx00='"+xrdx00+"'").list();
//            	if(pernum.size()<=0) {
				sql="delete from edu_xrdx_ry where xrdx00='"+xrdx00+"' and a0000='"+hm.get("a0000")+"'";
				stmt.executeUpdate(sql);
				sql="insert into edu_xrdx_ry(xrdx00,a0000,sortid) values ('"+xrdx00+"','"+hm.get("a0000")+"',"+i+")";
				stmt.executeUpdate(sql);
				i++;
...
//            	}
	...
	public int exists(String a0000,String xrdx00,HBSession session){
    	@SuppressWarnings("unchecked")
		List<String> exists= HBUtil.getHBSession().createSQLQuery("select a0000 from edu_xrdx_ry where a0000='"+a0000+"'  and xrdx00='"+xrdx00+"' ").list();
    	if(exists.size()>0) {
    		return 1;
    	}
		return 0;
