...
public class ZjImpZzbPart1Thread implements Runnable {
	...
	public ZjImpZzbPart1Thread(UserVO userVo, String houzhui, String imprecordid,
	...
	public void run() {
		...
		try {
			if(houzhui.equalsIgnoreCase("7z")||houzhui.equalsIgnoreCase("zip")){
				String tables[] = { "A01", "A02", "A05", "A06", "A08", "A14", "A15", "A36", "A57", "B01", "A99Z1", "A30", "Extra_Tags", "A0193_Tag", "A0194_Tag", "Tag_rclx", "Tag_cjlx", "Tag_sbjysjl", "Tag_kcclfj", "Tag_kcclfj2", "Tag_ndkhdjbfj", "Tag_dazsbfj"};
				
				int time = tables.length;
				...
				if(DBUtil.getDBType().equals(DBType.MYSQL)){
					...
					while (true) {
                		...
                		String tables[] = { "A01", "A02", "A06", "A08", "A14", "A15", "A36", "A57", "A29", "B01", "A30", "I_E", "B_E", "A05", "A99Z1", "Extra_Tags", "A0193_Tag", "A0194_Tag", "Tag_rclx", "Tag_cjlx", "Tag_sbjysjl", "Tag_kcclfj", "Tag_kcclfj2", "Tag_ndkhdjbfj", "Tag_dazsbfj" };
                		int time = houzhui.equalsIgnoreCase("hzb") ? tables.length : 14;
			...
			if (control.getStatus() == 2) {
				...
				KingbsconfigBS.saveImpDetail(process_run, "1", "正在清除A30——重复数据",imprecordid);
				CommonQueryBS.systemOut("正在清除A30——重复数据  ---start");
				String valueFromTab = HBUtil.getValueFromTab("count(1)", "a30"+tableExt, "1=1");
				if(!"0".equals(valueFromTab)){
					try{
						sess.createSQLQuery("drop table tmp2019").executeUpdate();
					}catch(Exception e){}
					sess.createSQLQuery("create table tmp2019 as select distinct a.* from a30"+tableExt+" a").executeUpdate();
					sess.createSQLQuery("delete from a30"+tableExt+"").executeUpdate();
					String num = HBUtil.getValueFromTab("count(1)", "tmp2019", "(a0000,a3004) not in (select a0000,max(a3004) from tmp2019 group by a0000)");
					if(!"0".equals(num)){//判断是否存在一人多条退出信息
						sess.createSQLQuery("delete from tmp2019 where (a0000,a3004) not in (select a0000,max(a3004) from tmp2019 group by a0000)").executeUpdate();
					}
					sess.createSQLQuery("insert into a30"+tableExt+" select A0000,A3001,A3004,A3117A,A3101,A3137,A3034,UPDATED,A3007A,A3038,'2' error_info from tmp2019").executeUpdate();
					sess.createSQLQuery("drop table tmp2019").executeUpdate();
				}
				CommonQueryBS.systemOut("正在清除A30——重复数据  ---end");
				...
				if("false".equals(fxz)){
					...
					if(houzhui.equalsIgnoreCase("7z")||houzhui.equalsIgnoreCase("zip")){
						String tables[] = { "A02", "A05", "A06", "A08", "A14", "A15", "A36", "A57", "A99Z1", "A01", "Extra_Tags", "A0193_Tag", "A0194_Tag", "Tag_rclx", "Tag_cjlx", "Tag_sbjysjl", "Tag_kcclfj", "Tag_kcclfj2", "Tag_ndkhdjbfj", "Tag_dazsbfj" };
						for(int i=0;i<tables.length;i++){
					    	...
					    	String tables[] = { "A02", "A06", "A08", "A11", "A14", "A15", "A29", "A30", "A31", "A36", "A37", "A41", "A53", "A57", "I_E", "A60", "A61", "A62", "A63", "A64", "A05", "A68", "A69", "A71", "A99Z1", "A01", "Extra_Tags", "A0193_Tag", "A0194_Tag", "Tag_rclx", "Tag_cjlx", "Tag_sbjysjl", "Tag_kcclfj", "Tag_kcclfj2", "Tag_ndkhdjbfj", "Tag_dazsbfj" };
						for(int i=0;i<tables.length;i++){
							try {
								sess.createSQLQuery("delete from "+tables[i]+tableExt+" where A0000 in ("+fxzSql+")").executeUpdate();
							} catch (Exception e) {
