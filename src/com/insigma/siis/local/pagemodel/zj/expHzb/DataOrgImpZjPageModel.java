...
import com.insigma.siis.local.pagemodel.dataverify.DataOrgExpNewThread;
import com.insigma.siis.local.pagemodel.dataverify.UploadHelpFileServlet;

public class DataOrgImpZjPageModel extends PageModel {
	...
	try {
	    ...
	    KingbsconfigBS.saveImpDetailInit3(id);
	    UserVO userVo = PrivilegeManager.getInstance().getCueLoginUser();
	    DataOrgExpNewThread thr = new DataOrgExpNewThread(id, user, gjgs, fxzry, gzlbry, gllbry, searchDeptid, linkpsn, linktel, remark, gz_lb, gl_lb, tabimp, zdcjg, userVo, sign);
	    new Thread(thr, "数据导出线程1").start();
	    this.setRadow_parent_data(id);
