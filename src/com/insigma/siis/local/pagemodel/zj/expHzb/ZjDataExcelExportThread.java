...
public class ZjDataExcelExportThread {
	...
	public ZjDataExcelExportThread(String mbpath,String uuid,CurrentUser user,String fxzry,String gzlbry,String gllbry,String searchDeptid,
	...
	public void copyFile(String sourcefile, String targetDir,String nameIc) throws IOException {
		try{
			File sourceFile =  new File(ZjDataExcelExportThread.PHOTO_PATH +  sourcefile);
		...
		try{
			...
			if("true".equals(exptemplate)){
				...
				String codeValueSql = "SELECT c.CODE_TYPE,c.CODE_VALUE,c.CODE_NAME3 FROM CODE_VALUE c WHERE c.CODE_TYPE IN('ZB01','ZB87',"
						...
						+ "'ZB129','ZB122','ZB14','GB8561','ZB24','ZB64','GB6864','GB16835','ZB123','ZB03','ZB128','ZB18','ZB78',"
						+ "'ZBXDSGWCJ','ZBXDSLB','ZBXDSLY')";
				...
				if(DBType.ORACLE == DBUtil.getDBType()||DBUtil.getDBType().equals(DBType.ORACLE)){
					a0000SQL = a0000SQL + " and exists (select 1 from a02, competence_userdept s where"
							...
							+ " and s.b0111 like '"+searchDeptid+"%' "
							+ " and s.userid='"+userVo.getId()+"' "
							+ " and a01.a0000=a02.a0000 group by a02.a0000"
							+ " )";
				...
				for(int i=0;i<sheetNums;i++){
					...
					if("true".equals(expsy)){//导出人员
						if(i==2){
							...
							exportData11(sheet,workbook, cellstyle,tableExt,codeValues);
			...
			if("true".equals(exppicture)){
				...
				CommonQueryBS.systemOut("导出图片路径------"+ZjDataExcelExportThread.PHOTO_PATH+picpath);
	...
	public void exportData10(Sheet sheet,Workbook workbook,CellStyle cellstyle,String tableExt,Map<String, String> codeValues) throws Exception{
		try{
			...
			for(int i=0;i<list.size();i++){
				...
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue(codeValues.get("ZBXDSGWCJ"+(String)map.get("xdsgwcj")));//选调生岗位层级
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue((String)map.get("dbjjggzsj"));//到本级机关工作时间
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue(codeValues.get("ZBXDSLB"+(String)map.get("a99zxdslb")));//选调生类别
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue((String)map.get("xdsdwjzw"));//选调时单位及职务
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue(codeValues.get("ZBXDSLY"+(String)map.get("xdsly")));//选调生来源
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue((String)map.get("dsjggzsj"));//地市机关工作时间（X年）
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue((String)map.get("jcgzjlsj"));//基层工作经历时间（X年）
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue((String)map.get("xxmc"));//学校名称(无选项可手填)
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue((String)map.get("zy"));//专业(无选项可手填)
			}
		}catch(Exception e){
			e.printStackTrace();
			throw new Exception(e.getMessage());
	...
	public void exportData11(Sheet sheet,Workbook workbook,CellStyle cellstyle,String tableExt,Map<String, String> codeValues) throws Exception{
		try{
			CommQuery cq=new CommQuery();
			String sql="select t.A0101, "
					 + " t.A0184, "
					 ...
					 + "  a30.A3007A, "
					 + " a30.A3038 "
					 + " from a30 a30,TEMP"+tableExt+" t "
					 + " where t.a0000 = a30.a0000 ";
			List<HashMap<String, Object>>  list=cq.getListBySQL(sql);
			String unitname="";
			...
			for(int i=0;i<list.size();i++){
				...
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue((String)map.get("a0101"));  //姓名
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue((String)map.get("a0184"));  //身份证号
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue(codeValues.get("ZB78"+(String)map.get("a3001")));//退出方式
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue((String)map.get("a3004"));//
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue((String)map.get("a3007a"));//退出去向
				cell=row.createCell(m++);cell.setCellStyle(cellstyle);cell.setCellValue((String)map.get("a3038"));//批准单位
			}
		}catch(Exception e){
			e.printStackTrace();
			throw new Exception(e.getMessage());
