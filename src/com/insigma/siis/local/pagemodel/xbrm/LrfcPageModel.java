...
public class LrfcPageModel extends PageModel {

	@Override
	public int doInit() throws RadowException {
		this.setNextEventName("yc");	
	...
	public int dogridquery(int start,int limit) throws Exception{
		...
		String sql2 = "select *" + 
				...
				"   and months_between(to_date('"+warndate+"','yyyy-mm-dd')," + 
				"                                       to_date(substr(b.a0243, 0, 6) || '01'," + 
				"                                               'yyyymmdd'))>= 120-to_number('"+twodate+"') " + 
				"   and a.a0221 = '"+gwcode+"'  " + 
				"   and b.a0201b like '"+ereaid+"%'";
        ...
        String sql3 = "select *" + 
        		...
        		"                                       'yyyymmdd')) <= 180 " + 
        		"   and months_between(to_date('2019-08-27', 'yyyy-mm-dd')," + 
        		"                               to_date(substr(nvl(c.a0504,replace('"+warndate+"','-','')), 0, 6) || '01'," + 
        		"                                       'yyyymmdd')) >= 180 - to_number('"+threedate+"') " + 
        		"   and a.a0221 = '"+gwcode+"' " + 
        		"   and b.a0201b like '"+ereaid+"%'";   
        
        if("第一任期到期".equals(seriesName)) {
        	sql = sql1;
        }else if("第二任期到期".equals(seriesName)){
        	sql = sql2;
        }else if("同一层次满15年".equals(seriesName)){
        	sql = sql3;
        	//this.getExecuteSG().addExecuteCode("hide1()");
        }
	 ...
	 */
	@PageEvent("yc")
	public int yc() throws RadowException, Exception {
		//this.setNextEventName("grid1.dogridquery");
		String seriesName = this.getPageElement("seriesName").getValue();
		if("第一任期到期".equals(seriesName)) {
			this.getExecuteSG().addExecuteCode("hide3()");
        }else if("第二任期到期".equals(seriesName)){
        	this.getExecuteSG().addExecuteCode("hide3()");
        }else if("同一层次满15年".equals(seriesName)){        	
        	this.getExecuteSG().addExecuteCode("hide4()");
        }
		return EventRtnType.NORMAL_SUCCESS;
