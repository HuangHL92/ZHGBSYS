...
public class LsmdByNamePageModel extends PageModel {
...
//						unionsql = unionsql +" and (a01."+col1+" like '%"+name+"%' or a01."+col2+" like '"+name.toUpperCase()+"%')";
	...
	public int saveSelect(String a) throws RadowException, AppException{
		...
		try {
			...
			List<String> maxnumber= HBUtil.getHBSession().createSQLQuery("select sortid from (select nvl(sortid,0) sortid from historyPer where mdid='"+mdid+"' order by nvl(sortid,0) desc) where rownum=1").list();
			int maxn;
...
//				addlist.add(hm.get("a0000")+"");
				sql="delete from historyPer where mdid='"+mdid+"' and a0000='"+hm.get("a0000")+"' ";
				stmt.executeUpdate(sql);
				
				maxn=maxn+1;
				sql="insert into historyPer (a0000,a0101,a0104,a0117,a0111a,a0134,a0144,a0141,a0221,a0196,a0107,a0192a,a0192f,qrzxlxw,qrzxlxwxx,zzxlxw,zzxlxwxx,mdid,sortid,id,a0184,a0193,a1705,a0188)"
						+ "  select a0000,a0101,a0104,a0117,a0111a,a0134,a0144,a0141,a0221,a0196,a0107,a0192a,substr(replace(a0192f,'.',''),0,6),"+ids+",'"+mdid+"',"+maxn+",sys_guid(),a0184,'' a0193,'' a1705,'"+userid+"' from "
								+ " a01  where a0000 ='"+hm.get("a0000")+"'";
				stmt.executeUpdate(sql);
			}
			stmt.close();
		...
		this.getExecuteSG().addExecuteCode("window.close();parent.updatePersonGrid();");
