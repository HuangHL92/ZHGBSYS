...
//	}
	...
	public int query(String id) throws RadowException {
		...
		this.setNextEventName("person.dogridquery");
		return EventRtnType.NORMAL_SUCCESS;
	}
	...
	//查出机构下的人员
	@PageEvent("person.dogridquery")
	public int personquery(int start,int limit)throws RadowException{
		String groupid = this.getPageElement("groupid").getValue();
		String sql = "select a01.a0000,A0101,A0184 from a01,a02 where a01.a0000=a02.a0000 and a0201b like '"+groupid+"' and a0255='1'";
	...
	public int transferSysOrg() throws RadowException, AppException {
		...
		if(transferType.equals("transferSysOrg")){//transferSysOrg隶属关系变更
			...
			temp=temp+" ) and ";
			String conutsql = "select count(t.a0000) from (select a0000 from a02 where "+temp+" a0255='1' group by a0000) t";
			...
			dialog_set("transferSysOrg.move","确认将"+turnOutSysOrg+"机构下所选人员转入"+changeIntoSysOrg+"机构下吗？");
		}
		return EventRtnType.NORMAL_SUCCESS;
	}
	...
	public int batchTransferPersonnelBypeople(String outId,String inId) throws RadowException{
		String a0000sid = this.getPageElement("a0000s").getValue();//得到操作的人员'xxxxxxxxxxxxxx','bbbbbbbbbbbb'
		...
		if(DBType.ORACLE==DBUtil.getDBType()){
			...
			"'','','','','','','99','','','','','','','','','1','','','','','','','','','',A0281,'','','','','','','','',"+
			"'','','','','' from a02 where a0201b='"+outId+"' and a0255='1' and a02.a0000 in ("+a0000sid+")";
		}else{
			insertsql = "insert into a02(A0000,A0200,A0201,A0201A,A0201B,A0201C,A0201D,A0201E,A0204,A0207,A0209,A0215A," +
			"A0215B,A0216A,A0219,A0219W,A0221,A0221W,A0222,A0223,A0225,A0229,A0243,A0245,A0247,A0251,A0251B,A0255,A0256," +
			"A0256A,A0256B,A0256C,A0259,A0265,A0267,A0271,A0277,A0281,A0284,A0288,A0289,A0295,A0299,A4901,A4904,A4907," +
			...
			"NULL,NULL,NULL,NULL,NULL from a02 where a0201b='"+outId+"' and a0255='1' and a02.a0000 in ("+a0000sid+")";
		}
...
//		}
		...
		String sql="update a02 set a0255 = '0' , a0281='false' "//人员任职状态以，a0281 为准，a0255可有可无
				+ " where a0201b='"+outId+"' and a0255='1' and a02.a0000 in ("+a0000sid+")";//将原任职信息记录的机构状态更新为已免
		String conutsql = "select count(t.a0000) from (select a0000 from a02 where a0201b='"+outId+"' and a0255='1' and a02.a0000 in ("+a0000sid+") group by a0000) t";
		Object c = HBUtil.getHBSession().createSQLQuery(conutsql).uniqueResult();
		int count = HBUtil.getHBSession().createSQLQuery(sql).executeUpdate();
		WorkUnitsAddPagePageModel workUnitsAddPagePageModel =new WorkUnitsAddPagePageModel();
		String a0000s="select t.a0000 from a01 t,a02 b where t.a0000=b.a0000 and  b.a0201b = '"+inId+"' and b.a0000 in ("+a0000sid+")";
	...
	public int batchTransferPersonnelBypeople1(String outId,String inId) throws RadowException{
	   ...
	   //String sql="update a02 set a0201a='"+b01.getB0101()+"', a0201c='"+b01.getB0104()+"', a0201b = '"+inId+"' where a0201b='"+outId+"' and a0255='1'";
		String sql="update a02 set a0201a = (select b0101 from b01 where b0111='"+inId+"') , a0201b='"+inId+"' "//更新任职信息
				+ " where a0201b='"+outId+"' and a0255='1' and a02.a0000 in ("+a0000sid+")";//将在任的人员更新任职信息和任职代码
		String conutsql = "select count(t.a0000) from (select a0000 from a02 where a0201b='"+outId+"' and a0255='1' and a02.a0000 in ("+a0000sid+")group by a0000) t";
		Object c = HBUtil.getHBSession().createSQLQuery(conutsql).uniqueResult();
		int count = HBUtil.getHBSession().createSQLQuery(sql).executeUpdate();
		WorkUnitsAddPagePageModel workUnitsAddPagePageModel =new WorkUnitsAddPagePageModel();
	...
	public void closeCueWindow(String arg0) {
		...
		this.getExecuteSG().addExecuteCode("parent.parent.odin.ext.getCmp('" + arg0 + "').close();");;
	}
