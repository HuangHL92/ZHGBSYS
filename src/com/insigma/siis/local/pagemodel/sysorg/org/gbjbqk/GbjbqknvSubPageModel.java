...
public class GbjbqknvSubPageModel extends PageModel{

	public GbjbqknvSubPageModel() {
	}
	    	...
	    	//列条件
	          for(int i=0;i<param_arr_col.length;i++){
	        	  if("3".equals(param_arr_col[i])){//合计
	 	        	 ...
	 	        	 sb.append(" and " +GbjbqkSubPageModel.col[Integer.valueOf(param_arr_col[i])+2]);
	 	         }
	          }
			...
			//列条件
	          for(int i=0;i<param_arr_col.length;i++){
	        	  if("3".equals(param_arr_col[i])){//合计
	 	        	 ...
	 	        	 sb.append(" and " +GbjbqkSubPageModel.col_mysql[Integer.valueOf(param_arr_col[i])+2]);
	 	         }
	          }
		}else{
	...
	public int init() throws RadowException, UnsupportedEncodingException, AppException{
		...
		GbjbqkSqlZs gbjbqksqlzs=new GbjbqkSqlZs();
		//拼接sql select
		gbjbqksqlzs.returnGbjbqkSqlZs1(sb,"nv");
		//拼接sql from
		//gbjbqksqlzs.returnGbjbqkSqlZs11(sb);
		gbjbqksqlzs.returnGbjbqkSqlZs11_sub(sb,xueli,xuewei);
		gbjbqksqlpj.sqlPjExists(sb, groupid);//机构id
		//拼接where条件
		if(DBType.ORACLE==DBUtil.getDBType()){//oracle 数据源
			sb.append(" and nvl(a01.a0104,0)='2' ");//性别 女2
		}else if(DBType.MYSQL==DBUtil.getDBType()){//mysql数据源
			sb.append(" and if(a01.a0104='',0,a01.a0104)='2' ");//性别 女2
		}else{
			this.setMainMessage("未知数据源，请联系系统管理员!");
			return EventRtnType.NORMAL_SUCCESS;
		}
		//条件 a0221 过滤
  	    gbjbqksqlpj.sqlPjA01(sb);
		...
		gbjbqksqlpj.sqlPjExists(sb1, groupid);
		 //拼接where条件
		if(DBType.ORACLE==DBUtil.getDBType()){//oracle 数据源
			sb1.append(" and nvl(a01.a0104,0)='2' ");//性别 女2
		}else if(DBType.MYSQL==DBUtil.getDBType()){//mysql数据源
			sb1.append(" and if(a01.a0104='',0,a01.a0104)='2' ");//性别 女2
		}else{
			this.setMainMessage("未知数据源，请联系系统管理员!");
			return EventRtnType.NORMAL_SUCCESS;
		}
		...
		if(DBType.ORACLE==DBUtil.getDBType()){
			...
			for(int i=0;i<param_arr_col.length;i++){
				if("3".equals(param_arr_col[i])){//合计
			       	 ...
			       	 sb1.append(" and " +GbjbqkSubPageModel.col[Integer.valueOf(param_arr_col[i])+2]);
			     }
			}
		}else if(DBType.MYSQL==DBUtil.getDBType()){
			//列条件
			for(int i=0;i<param_arr_col.length;i++){
				if("3".equals(param_arr_col[i])){//总计
			       	 
			     }else{
			       	 sb1.append(" and " +GbjbqkSubPageModel.col_mysql[Integer.valueOf(param_arr_col[i])+2]);
			     }
			}
		}else{
			throw new RadowException("发现未知数据源，请联系系统管理员!");
