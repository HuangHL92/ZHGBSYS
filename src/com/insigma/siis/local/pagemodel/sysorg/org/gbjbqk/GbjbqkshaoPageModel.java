...
public class GbjbqkshaoPageModel extends PageModel{
	...
	public int zwxxQuery(int start, int limit) throws RadowException {
	    ...
	    if(DBType.ORACLE==DBUtil.getDBType()){//oracle 数据源
	    	sb.append(" and nvl(a01.a0117,0)!='01' ");//少数民族 
	    }else if(DBType.MYSQL==DBUtil.getDBType()){//mysql数据源
	    	sb.append(" and if(a01.a0117='',0,a01.a0117)!='01' ");//少数民族 
	    }else{
	...
	public int init() throws RadowException, UnsupportedEncodingException, AppException{
		...
		//拼接where条件
		if(DBType.ORACLE==DBUtil.getDBType()){//oracle 数据源
			sb1.append(" and nvl(a01.a0117,0)!='01' ");//少数民族 
		}else if(DBType.MYSQL==DBUtil.getDBType()){//mysql数据源
			sb1.append(" and if(a01.a0117='',0,a01.a0117)='2' ");//性别 女2
		}else{
			this.setMainMessage("未知数据源，请联系系统管理员!");
			return EventRtnType.NORMAL_SUCCESS;
		}
