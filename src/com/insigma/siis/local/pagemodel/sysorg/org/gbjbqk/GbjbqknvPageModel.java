...
public class GbjbqknvPageModel extends PageModel{
	...
	public int zwxxQuery(int start, int limit) throws RadowException {
		...
		//a02 与权限表
		gbjbqksqlpj.sqlPjExists(sb, groupid); //条件 a0000 exists  a02表中
		if(DBType.ORACLE==DBUtil.getDBType()){//oracle 数据源
			...
			this.setMainMessage("发现未知数据源，请联系系统管理员!");
			return EventRtnType.NORMAL_SUCCESS;
		}
	...
	public int init() throws RadowException, UnsupportedEncodingException, AppException{
		...
		GbjbqkSqlPj gbjbqksqlpj=new GbjbqkSqlPj();
		gbjbqksqlzs.returnGbjbqkSqlZs1(sb,"nv");//拼接sql select
		gbjbqksqlzs.returnGbjbqkSqlZs11(sb);//拼接sql from
		...
		gbjbqksqlzs.returnGbjbqkSqlZs2(sb1);//拼接sql select
		gbjbqksqlzs.returnGbjbqkSqlZs21(sb1);//拼接sqlfrom
		gbjbqksqlpj.sqlPjExists(sb1, groupid);//a02 与权限表
		 //拼接where条件
		if(DBType.ORACLE==DBUtil.getDBType()){//oracle 数据源
