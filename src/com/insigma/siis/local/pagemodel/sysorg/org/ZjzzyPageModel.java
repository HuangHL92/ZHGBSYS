...
//	}
	...
	//			} 
				...
				/***********机构排序，新字段 2020年11月12日10:08:31 zoul********3位点3位，后面调整最后加1位字母校验位*******************************************************************/
				String sqlb0269 = "select max(B0269) from B01 where b0121='"+list2.get(0).getB0111()+"'";
				Object maxsort = HBUtil.getHBSession().createSQLQuery(sqlb0269).uniqueResult();
				String maxsortNUM = "";
				if(maxsort!=null&&!"".equals(maxsort.toString())){
					...
					if(num.length()==4){
						...
						maxsortNUM = maxsort.toString().substring(0,maxsort.toString().length()-4)+StringUtils.leftPad((maxsortNUM), 4, "0");
					}else{
						maxsortNUM = (Integer.valueOf(num)+1)+"";
						maxsortNUM = maxsort.toString().substring(0,maxsort.toString().length()-3)+StringUtils.leftPad((maxsortNUM), 3, "0");
					}
					
				}else{
					maxsortNUM = HBUtil.getValueFromTab("B0269", "b01", "b0111='"+list2.get(0).getB0111()+"'")+".001A";
				}
				/***********机构排序，新字段 2020年11月12日10:08:31 zoul********3位点3位，后面调整最后加1位字母校验位*******************************************************************/ 
				...
				for(int i=0;i<outId.length;i++){
					...
					String sql6="update b01 t set t.b0121='"+changeIntoSysOrg+"',t.sortid='"+sortid+"',t.b0111='"+b0111+"',t.b0269='"+maxsortNUM+"' where t.b0111='"+outId[i]+"'";
					...
					/***********机构排序，新字段 2020年11月12日10:08:31 zoul********3位点3位，后面调整最后加1位字母校验位*******************************************************************/
					OrgSortPageModel.moveOrgSort(b01temp.getB0269(), maxsortNUM);
					/***********机构排序，新字段 2020年11月12日10:08:31 zoul********3位点3位，后面调整最后加1位字母校验位*******************************************************************/
