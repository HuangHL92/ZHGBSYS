...
public class ViewConditionPageModel extends PageModel{
...
//		this.createPageElement("conditionName71", ElementType.SELECT, false).setDisabled(true);
	...
	public int init() throws RadowException{
		String paramurl=this.getPageElement("paramurl").getValue();
	    if(paramurl!=null &&paramurl.trim().equals("UserDefinedQuery")){//人员信息，自定义查询
...
//			String querysql=HBUtil.getValueFromTab("qrysql", "qryview", "qvid='"+qvid+"'");
...
//			}
			...
			String sql=""
					...
					+ " from qryuse q where qvid='"+qvid+"' " + " order by sort asc " ;
			List<HashMap<String, Object>> list=cq.getListBySQL(sql);//3个条件具体信息
			
			String zhtj=HBUtil.getValueFromTab("qryview.conditions", "qryview", "qvid='"+qvid+"' ");// 1.并且. 2.并且. 3
	...
	public int initListAndButton() throws RadowException, AppException{
		...
		if(qvid!=null&&!qvid.trim().equals("")&&!qvid.trim().equals("null")){
			...
			String sql=" select tblname from ("
					+ " select tblname from qryviewfld where qvid='"+qvid+"'"
					+ " union all"
					+ " select tblname from qryuse where qvid='"+qvid+"' "
					+ " ) tt group by tblname ";
			CommQuery cq=new CommQuery();
			...
			List<HashMap<String, Object>> listfld=cq.getListBySQL("select tblname,fldname from qryviewfld where qvid='"+qvid+"'");
			String flds="";
...
//		.replace("，", "")
				...
				if("".equals(str)){//无选中表时
					...
					if(!flds.isEmpty()){
						String sqlnew=" select concat(concat(concat(concat(c.table_code,'.'),c.col_code),' '),c.col_name) col_name1,"
								...
								+ " c.table_code, q.FLDNUM from qryviewfld q left join code_table_col c  on c.COL_CODE=q.FLDNAME"
								+ " where 1 = 1 and "
								/*+ " ISUSE = 1 and "*/
								/*青岛特定数据库
								 * + " ISLOOK='1' and "*/
	...
	public int saveFunc(String v) throws RadowException{
		...
		boolean flagviewtb=false;//判断更新，新增操作
		try{
			paramurl=this.getPageElement("paramurl").getValue();
			qvid=this.getPageElement("qvid").getValue();
			
			qvsql=HBUtil.getValueFromTab("qrysql", "qryview", "qvid='"+qvid+"'");
			...
			try{
				...
				Qryview qryview=(Qryview)hbSession.get(Qryview.class, qvid);
				viewnametb=qryview.getViewnametb();
				if(viewnametb==null||"null".equals(viewnametb.trim())||"".equals(viewnametb.trim())){
					//库中视图名称
					viewnametb=HUtil.getSequence("sq_qryview");
					viewnametb="V_qv"+viewnametb;
					qryview.setViewnametb(viewnametb);
					flagviewtb=true;//true 新增
				}
				qryview.setQrysql(qrysql);
				qryview.setConditions(zhtj);
				hbSession.update(qryview);//保存sql到视图表中
				hbSession.flush();
		//	}
			stmt =connection.createStatement();
			viewSql="create or replace view "+viewnametb+" as "+qrysql;
			...
			for(int i=0;i<listCode1.size();i++){
				...
				sql="insert into qryviewfld (qvfid,qvid,tblname,fldname,fldnamenote,fldnum ) "
						+ " values ("+CommonSQLUtil.UUID()+",'"+qvid+"','"+table_code+"','"+col_code+"','"+col_name+"','"+fldnum+"')";
				stmt.execute(sql);
			}
			if(a0000flag==false){
				String sqlInsertA0000="insert into qryviewfld (qvfid,qvid,tblname,fldname,fldnamenote,fldnum ) "
						+ " values ("+CommonSQLUtil.UUID()+",'"+qvid+"','"+"A01"+"','"+"A0000"+"','"+"人员统一标识符"+"','0')";
				stmt.execute(sqlInsertA0000);
			}
			if(a0101flag==false){
				String sqlInsertA0101="insert into qryviewfld (qvfid,qvid,tblname,fldname,fldnamenote,fldnum ) "
...
//				String uuid=UUID.randomUUID().toString();
				sqlInsertQryuse="insert into qryuse (quid,qvid,sort,fldname,fldcode,"
			...
			if(qvsql==null||qvid.trim().equals("qvsql")||qvid.trim().equals("")){//新增视图
				if(!"refreshPreviewTab".equals(v)){
					this.setMainMessage("保存成功!");
				}else{
					this.getExecuteSG().addExecuteCode("parent.refreshPreviewTab2();");
				}
			...
			try{
				if(flagviewtb==true){//新增保存，则删除保存的数据（创建视图 不能回滚数据）
					deleteErr(qvid,viewnametb,paramurl);
				}
			}catch(Exception e1){
	...
	public String getSql() throws RadowException{
		...
		for(int i=0;i<listCode.size();i++){
			String checked=listCode.get(i).get("checked")+"";
			if("true".equals(checked)){
				String temp=(String)listCode.get(i).get("col_code");
				if("a0000".equals( temp.toLowerCase() )){
					flag=true;
				}
			}
		}
		for(int i=0;i<listCode.size();i++){
			...
			if("true".equals(checked)){
				String temp=(String)listCode.get(i).get("col_code");
				if("a0101".equals( temp.toLowerCase() )){
					a0101flag=true;
				}
			}
		}*/
...
//			String checked=listCode.get(i).get("checked")+"";
//			if("true".equals(checked)){
//				String temp=(String)listCode.get(i).get("table_code");
...
//			}
		...
		sqlSelect=sqlSelect+" where 1=1 and a01.fkly is null ";
		String tableid="";//表关联
	...
	public int deleteErr(String qvid,String viewnametb,String paramurl){
		...
		try{
			...
			if(paramurl!=null&&paramurl.trim().length()>0&&!paramurl.equals("null")){//自定义查询（人员信息查询tab），仅信息查询界面可以删除 
				stmt =connection.createStatement();
				stmt.execute("delete from qryview where qvid='"+qvid+"' ");
				stmt.close();
			}
			if(qvid!=null&&qvid.trim().length()>0){
				stmt =connection.createStatement();
				stmt.execute("update qryview set qrysql = '' where qvid='"+qvid+"' ");
				stmt.close();
			}
