...
public class UsersRootPageModel extends PageModel {
...
//			" {text: '"+map.get("b0101")+"',id:'"+map.get("b0111")+"',leaf:false,icon:'"+icon+"',\"href\":\"javascript:if('true'=='"+flag+"'){alert('您没有该机构的权限')}else{radow.doEvent('specoper')}\"}"
	...
	@NoRequiredValidate
	public int chooseUser(String userid)throws RadowException, AppException{
		//String userid = this.getPageElement("subWinIdBussessId").getValue();
		if(userid==null||"".equals(userid)){
			this.setMainMessage("请先选择要更改的用户");
		}
		HBSession sess = HBUtil.getHBSession();
		Object obj = sess.createSQLQuery("select count(*) from smt_user where userid = '"+userid+"' and userid not in('U001','4028810f5f2aed67015f2aefeeee0002')").uniqueResult();
		int i = Integer.parseInt(""+obj);
		if(i==0){
			this.getPageElement("loginname").setValue("");
			this.getPageElement("username").setValue("");
			this.getPageElement("work").setValue("");
			this.getPageElement("mobile").setValue("");
			this.getPageElement("tel").setValue("");
			this.getPageElement("email").setValue("");
			this.getPageElement("usertype").setValue("");
			this.getPageElement("validity").setValue("");
			this.getPageElement("status").setValue("");
		}else{
			...
			String logonfailtimes = HBUtil.getValueFromTab("logonfailtimes", "smt_user", "loginname='"+user.getLoginname()+"'");
			Long lft = logonfailtimes==null?0l:Long.valueOf(logonfailtimes.toString());
			if(lft>=5){
				this.getPageElement("status").setValue("锁定");
			}else{
				this.getPageElement("status").setValue("正常");
			}
			...
			String validity = "";
			if("validity01".equals(user.getValidity())){
				validity = "一周";
			}else if("validity02".equals(user.getValidity())){
				validity = "一月";
			}else{
				validity = "";
			}
			this.getPageElement("validity").setValue(validity);
...
//		if("U000".equals(userid)||"U001".equals(userid)||"40288103556cc97701556d629135000f".equals(userid)||"4028810f5f2aed67015f2aefeeee0002".equals(userid)){
...
//		}f
	...
	@com.insigma.odin.framework.radow.annotation.Transaction
	public int deleteTrue(String groupid) throws RadowException, SQLException{
		HBSession sess = HBUtil.getHBSession();
		//删除本组下面的所有人员与组信息
	...
	private void deleteForGroup(HBSession sess,String groupid) throws RadowException, SQLException{
		//删除用户组下面的所有人员
		List<Object> list = sess.createSQLQuery("select t.userid from SMT_USER t where t.dept = '"+groupid+"' and useful='1'").list();
		if(list!=null&&list.size()>0){
			Date date = new Date();
			...
			for(Object obj : list){
				if(obj!=null){
					...
					try {
						UserVO user = new UserVO();
						user = PrivilegeManager.getInstance().getIUserControl().findUserByUserId(obj.toString());
						new LogUtil().createLogNew(user.getId(),"删除用户","smt_user",user.getId(),user.getLoginname(), new ArrayList());
					} catch (Exception e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
					//删除干部教育、干部监督的用户
					//deleteForJDJY(sess,""+obj);
				}
			}
		}
		try {
			new LogUtil().createLogNew(groupid,"删除用户组","smt_usergroup",groupid,HBUtil.getValueFromTab("usergroupname", "smt_usergroup", "id = '"+groupid+"'"), new ArrayList());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public int deleteUser() throws RadowException, PrivilegeException{
		...
		HBSession sess = HBUtil.getHBSession();
		Object obj = sess.createSQLQuery("select count(*) from smt_user where userid = '"+userid+"' ").uniqueResult();
		int i = Integer.parseInt(""+obj);
		if(i==0){
			this.setMainMessage("您好，请先选择要删除的用户");
			return EventRtnType.NORMAL_SUCCESS;
		...
		if("297e9b3367154ab2016716a77abe0ca2".equals(userid)||"402882846f6a49c9016f6a7d6e7b000a".equals(userid)){
			this.setMainMessage("您好，无法删除三元账号！");
			return EventRtnType.NORMAL_SUCCESS;
		}
		/*//判断用户授权跨组的问题
		if(!StrideUser()){
	...
	@com.insigma.odin.framework.radow.annotation.Transaction
	public int doRemoveUser(String userid) throws RadowException, PrivilegeException, SQLException{
		HBSession sess = HBUtil.getHBSession();
		...
		try {
			new LogUtil().createLogNew(userid,"删除用户","smt_user",userid,user.getLoginname(), new ArrayList());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		//删除干部教育、干部监督的用户
		//deleteForJDJY(sess,userid);

		this.getExecuteSG().addExecuteCode("deleteUserSuccess();");
		return EventRtnType.NORMAL_SUCCESS;
	}
	
	private void deleteForJDJY(HBSession sess, String cueUserID) throws RadowException, SQLException {
		Connection conn = sess.connection();
	...
	private void deleteAuthority(HBSession sess,String userid){
		...
		//删除用户与平台的关系
		sess.createSQLQuery("delete from SMT_PLATFORM where userid = '"+userid+"'").executeUpdate();
	}
	...
	public int reset() throws RadowException, PrivilegeException{
		...
		HBSession sess = HBUtil.getHBSession();
		//and userid not in('U001','40288103556cc97701556d629135000f','4028810f5f2aed67015f2aefeeee0002')
		Object obj = sess.createSQLQuery("select count(*) from smt_user where userid = '"+userid+"' ").uniqueResult();
		int i = Integer.parseInt(""+obj);
		if(i==0){
			//说明是部门
	...
	public int changeType() throws RadowException{
		...
		UserVO user=PrivilegeManager.getInstance().getCueLoginUser();
		if(!"1".equals(user.getUsertype())){
			throw new RadowException("只有系统管理员身份才能操作");
		}
		String userid = this.getPageElement("userid").getValue();
		if(userid==null||"".equals(userid)){
			this.setMainMessage("请先选择用户");
		...
		if(userid.equals(cueUserid)){
			this.setMainMessage("无法对自己进行操作");
			return EventRtnType.NORMAL_SUCCESS;
		}
		//判断用户授权跨组的问题
		/*if(!StrideUser()){
...
/*	private void inheritOtherPower(HBSession sess, String userid,
	...
	// 用户类型更改
	@PageEvent("unlock.onclick")
	public int unlock() throws RadowException, AppException{
		String cueUserid = SysUtil.getCacheCurrentUser().getId();
		String sql = "select validity,pwdedtdate,logonfailtimes,usertype  from smt_user where userid='"+cueUserid+"'";
		...
		map = li.get(0);
		if(map.get("usertype")!=null&&"1".equals(map.get("usertype").toString())){
			String userid = this.getPageElement("userid").getValue();
			...
			List ul = HBUtil.getHBSession().createSQLQuery("select 1 from smt_user where userid='"+userid+"'").list();
			if(ul.size()==0){
				this.setMainMessage("请先选择用户");
				return EventRtnType.NORMAL_SUCCESS;
			}
			HBUtil.executeUpdate("update smt_user set Logonfailtimes=0 where userid='"+userid+"'");
			LogUtil applog = new LogUtil();
			applog.createLogNew(userid,"用户登录解除锁定","smt_user",userid,loginname, new ArrayList());
		}else{
			this.setMainMessage("只有系统管理员可以操作！");
			return EventRtnType.NORMAL_SUCCESS;
		}
		EncodeA1701.encodeA1701();
		this.setMainMessage("解锁成功！");
		return EventRtnType.NORMAL_SUCCESS;
	}
	// 用户类型更改
	@PageEvent("lock.onclick")
	public int lock() throws RadowException, AppException{
		String cueUserid = SysUtil.getCacheCurrentUser().getId();
		String sql = "select validity,pwdedtdate,logonfailtimes,usertype  from smt_user where userid='"+cueUserid+"'";
		HBSession hs = HBUtil.getHBSession();
		CommQuery query = new CommQuery();
		...
		if(map.get("usertype")!=null&&"1".equals(map.get("usertype").toString())){
			...
			HBUtil.executeUpdate("update smt_user set Logonfailtimes=100 where userid='"+userid+"'");
			LogUtil applog = new LogUtil();
			applog.createLogNew(userid,"用户登录锁定","smt_user",userid,loginname, new ArrayList());
		}else{
			this.setMainMessage("只有系统管理员可以操作！");
			return EventRtnType.NORMAL_SUCCESS;
		}
		this.setMainMessage("锁定成功！");
		return EventRtnType.NORMAL_SUCCESS;
	}
