...
public class FJHZ2PageModel extends PageModel{
		...
		try {
			...
			if("ss2".equals(ss)){
				...
				if(spp05==null||"".equals(spp05)){
					throw new AppException("部领导意见不能为空！");
				}
				...
				sess.flush();
				//删除审批信息， 将审批信息添加到历史记录。
				sess.createSQLQuery("insert into SP_BUS_his (select * from SP_BUS where spb00='"+spp00+"')").executeUpdate();
				sess.delete(sb);
				sess.flush();
