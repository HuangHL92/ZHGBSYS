...
public class SPUtil {
	 ...
	 */
	public static String[][] nextJGLDconfig = new String[5][];
	/**
	 * 0环节 : 1当前审批结果 ：2审批状态 : 3描述 ：4审批机构是否可为空  : 5备注是否可为空 : 
	 */
	public static String[][] prevJGLDconfig = new String[5][];
	static{
		//0环节 : 1当前审批结果 ：2审批状态 : 3描述 ：4审批机构是否可为空  : 5备注是否可为空 : 6业务意见是否可为空
		nextJGLDconfig[1] = new String[]{"2","2","1","送审","0","1","1",""};//登记结束
		nextJGLDconfig[2] = new String[]{"3","3","1","审批通过","0","1","0","sp0111"};//一级审批结束
		nextJGLDconfig[3] = new String[]{"4","3","1","审批通过","0","1","0","sp0112"};//二级审批结束
		nextJGLDconfig[4] = new String[]{"5","3","2","审批通过(完结)","1","1","0","sp0113"};//三级审批结束
		
		//0环节 : 1当前审批结果 ：2审批状态 : 3描述 ：4审批机构是否可为空  : 5备注是否可为空 : 业务意见是否可为空
		prevJGLDconfig[2] = new String[]{"1","4","1","退回","1","0","1","sp0111"};//一级审批结束
		prevJGLDconfig[3] = new String[]{"2","4","1","退回","1","0","1","sp0112"};//二级审批结束
		prevJGLDconfig[4] = new String[]{"5","4","3","审批不通过(完结)","1","0","1","sp0113"};//三级审批结束
	}
	/**
	 * 通过01
	 * @param sp0100
	...
	public static void apply02(String sp0100, HBSession sess, Map<String, String> map,String type) throws NumberFormatException, AppException {
		...
		if("0".equals(type)){
			//审批流程表退回上一级
			String sql = "select t.spbl01 from SP_BUS_LOG t where spb00='"+sp0100+"' and t.spbl04 in('2','3') order by t.spbl00 desc";
