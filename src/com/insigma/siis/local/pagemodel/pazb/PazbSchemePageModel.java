...
public class PazbSchemePageModel extends PageModel{
	
	public PazbSchemePageModel(){
	}
...
//		List<Object> egList = hbSession.createSQLQuery("select * from PAKH_SCHEMES where ets01=?").setString(0, ets01).list();
//		if(egList.size()!=0){
//			this.setMainMessage("方案已存在!");
//			return EventRtnType.NORMAL_SUCCESS;
//		}
//		if (StringUtils.isEmpty(ets01)) {
...
//		}
        ...
        if(b01String!=null && !"".equals(b01String)&& !"{}".equals(b01String)){//tree!=null && !"".equals(tree.trim()
			...
			if(StringUtils.isEmpty(ets00)){//新增
				...
				HBUtil.executeUpdate("insert into PAKH_SCHEMES(ets00,ets01,ets02,ets04) values(?,?,seq_sort.nextval,?)",
						new Object[]{ets00,ets01,userID});
			}else{
				HBUtil.executeUpdate("update PAKH_SCHEMES set ets01=? where ets00=?",
						new Object[]{ets01,ets00});
			}
			String selsql = "select '"+ets00+"',b01id,b0269 from b01 where 1=1 "+ b0111_sb;
			HBUtil.executeUpdate("delete from PAKH_TARGET_ORG  where ets00=?",
					new Object[]{ets00});
			
			String insertSQL = "insert into PAKH_TARGET_ORG(ets00,b01id,b0269) ("+selsql+")";
	...
	public String getJson(String type,String nodeother,String sign){
		...
		if("system".equals(loginName)){//system用户不加权限
			...
			sql=" select ("+sql1+") count1, t.b0111,t.b0121,t.b0101,t.b0107,t.sortid,t.b0194,(select 'true' from PAKH_TARGET_ORG t1 where t1.b01id=t.b01id and t1.ets00='"+ets00+"') checked from "+b01Table+" t where t.b0121='"+node+"'  order by t.sortid asc ";			
		}else{
			...
			sql=" select ("+sql1+") count1, t.b0111,t.b0121,t.b0101,t.b0107,t.sortid,t.b0194,(select 'true' from PAKH_TARGET_ORG t1 where t1.b01id=t.b01id and t1.ets00='"+ets00+"') checked from "+b01Table+" t,"+deptTable+" s where t.b0111=s.b0111 and t.b0121='"+node+"' and s.userid='"+cueUserid+"' order by t.sortid asc ";
		}
