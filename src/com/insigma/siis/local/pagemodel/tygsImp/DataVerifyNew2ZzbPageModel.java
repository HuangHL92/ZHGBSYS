package com.insigma.siis.local.pagemodel.tygsImp;
...
public class DataVerifyNew2ZzbPageModel extends PageModel {
	...
	public int initX() throws RadowException, AppException {
		...
		try {
			...
			String unzip = upload_file + param1[0];// 解压路径
			...
			FileFilter filer = new MyFilter();
			File[] s = f.listFiles(filer);
			
			if(s==null){
				this.setMainMessage("请选择正确的ZIP文件！");
			...
			String linkpsn = map.get("linkpsn")!=null?map.get("linkpsn"):map.get("contact");// 根节点上级机构id
			String linktel = map.get("linktel")!=null?map.get("linkpsn"):map.get("tel");// 根节点上级机构id
