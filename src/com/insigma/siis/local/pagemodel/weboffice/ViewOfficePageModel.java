...
public class ViewOfficePageModel extends PageModel{
	...
	public int TrainingInfoGridQuery(int start,int limit) throws RadowException{
		...
		/*String  sql1="select officeid from COMPETENCE_USERWEBOFFICE where userid='"+cueUserid+"' ";
		List<Object[]> competence =  sess.createSQLQuery(sql1).list();*/
		
		//String sql = "from WebOffice  where type=3";
		//String sql = "select w.id,w.creattime,w.updatetime,w.filename,w.type from weboffice w,COMPETENCE_USERWEBOFFICE c where w.id=c.officeid  and c.userid='40288103556cc97701556d629135000f' and w.type='3'";
		//String sql = "from weboffice  w left join competence_userweboffice  c on w.id=c.officeid  where c.userid='"+cueUserid+"' and w.type=3";
		//String sql1="select w.id,w.filename,w.creattime,w.updatetime,w.selty ,c.userid from weboffice w left join competence_userweboffice c on w.id=c.officeid where c.userid='40288103556cc97701556d629135000f' and w.type=3 and w.selty=1";
		...
		/*}else{
			String sql = "select w.id,w.filename,w.creattime,w.updatetime from weboffice w left join competence_userweboffice c on w.id=c.officeid where c.userid='"+cueUserid+"' and w.type=3 and w.selty=0 or c.userid='"+cueUserid+"' and w.type=3 and w.selty=1";
