...
public class GroupSortPageModel extends PageModel{
	...
	public String getJson(String nodeother){
		...
		String sql ="select sg.GROUPID,sg.PARENTID,sg.NAME,sg.OWNER,sg.STATUS,sg.HASHCODE,sg.Rate from SMT_GROUP sg where 1=1 "+sql1 +" order by sg.sortid";
...
//		List<B01> list = null;//得到当前组织信息
	...
	@OpLog
	@Transaction
	public int YesNew(String ids) throws AppException, RadowException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, SQLException, IntrospectionException, PrivilegeException {
		...
		for(int i=0;i<arr.length;i++){
			SmtGroup smtgroup = (SmtGroup) sess.createQuery("From SmtGroup S where S.id = '"+arr[i]+"'").list().get(0);
			PropertyUtils.copyProperties(b01DTOb01ChooseOrgCod, smtgroup);
			smtgroup.setSortid(Long.parseLong(String.valueOf(i+1)));
			HashCodeUtil.getBeanHashCode(smtgroup);
			sess.update(smtgroup);
