		...
		for (Param param : params){
			...
			if (param.getName().equals("A0184") && !"".equals(param.getValue())) {
				...
				if(list==null||list.size()==0){
					...
					List<A61> a61s = HBUtil.getHBSession().createQuery(
							"from A61 where a0000='"+a0000+"'  ").list();
					
					if(a61s!=null&&a61s.size()>0){
						PreparedStatement pst = null;
						try {
							conn.setAutoCommit(false);
							pst = conn.prepareStatement("delete from a61 where a0000=?");
