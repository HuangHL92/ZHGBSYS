	...
	public static String save(List<Param> params,Connection conn) {
		...
		for (Param param : params) {
			...
			if (param.getName().equals("A0184") && !"".equals(param.getValue())) {
				...
				else{//查询是否已存在a29信息，如果已存在，先删除，后新增，  注意开启事务
					a0000 = list.get(0).getA0000();
					List<A30> a29s = HBUtil.getHBSession().createQuery(
							"from A29 where a0000='"+a0000+"'  ").list();
					
					if(a29s!=null&&a29s.size()>0){
						PreparedStatement pst = null;
						try {
							conn.setAutoCommit(false);
							pst = conn.prepareStatement("delete from a29 where a0000=?");
			...
			//补充
			if (param.getName().equals("A2950") || param.getName().equals("A2951")){
				if(param.getValue().equals("是")){
					param.setValue("1");
				}
				else if(param.getValue().equals("否")){
					param.setValue("0");
				}
				else{
					return "错误："+param.getName()+"值不合法！";
				}
			}
