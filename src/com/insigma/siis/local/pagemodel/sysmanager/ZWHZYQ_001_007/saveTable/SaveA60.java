	...
	public static String save(List<Param> params,Connection conn) throws AppException{
		...
		for (Param param : params){
			...
			if (param.getName().equals("A0184") && !"".equals(param.getValue())) {
				...
				}else{//查询是否已存在a60信息，如果已存在，先删除，后新增，  注意开启事务
					a0000 = list.get(0).getA0000();
					List<A60> a60s = HBUtil.getHBSession().createQuery(
							"from A60 where a0000='"+a0000+"'  ").list();
					
					if(a60s!=null&&a60s.size()>0){
						PreparedStatement pst = null;
						try {
							conn.setAutoCommit(false);
							pst = conn.prepareStatement("delete from a60 where a0000=?");
