package com.insigma.siis.local.pagemodel.gbmc.impexcel;
...
//		}
	...
	private List<Map<String, String>> changeList(List<Map<Integer, String>> rowList) throws Exception {
		...
		for (int i = 1; i < rowList.size(); i++) {
			...
			if ("1".equals(dataRow.get("type"))) {
				...
				// photoColumnKey
				dataRow.put("tp0117", (i + firstRowNum) + "-" + photoColumnKey);
			}
			result.add(dataRow);
		}
		return result;
	...
	private int save(List<Map<String, String>> rowList, Map<String, PictureData> pictureData) throws Exception {
		...
		String sql = "insert into TPHJ1(tp0100, a0000, type, tp0101,tp0117,tp0106,tp0118,tp0119,tp0120,tp0102,tp0121,tp0122,tp0123,tp0103,tp0104,tp0124,tp0125,tp0126,tp0127,tp0128, sortnum, yn_id,tp0116 )"
				+ " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
