...
public class Gbmc2 {
	//性别map
	static HashMap<String, String> xbMap = new HashMap<String, String>();
	//民族map
	static HashMap<String, String> mzMap = new HashMap<String, String>();
	
	static{
		xbMap.put("1", "男");
		xbMap.put("2", "女");
		HBSession session = HBUtil.getHBSession();
		List<CodeValue> list = session.createSQLQuery("select * from code_value c where c.code_type='GB3304'").addEntity(CodeValue.class).list();
		for (CodeValue codeValue : list) {
			mzMap.put(codeValue.getCodeValue(),codeValue.getCodeName());
		}
