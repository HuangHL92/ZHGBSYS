 ...
 *
 */
public class QueryConditionEditPageModel extends PageModel {
...
//		String tableName=RuleSqlListBS.getTableName(collection);  //不在保存汉字，保存代码
	...
	public int btnSaveOnclick() throws RadowException, AppException {
		...
		ctcBs.setSid(this.request.getSession().getId());
		...
		Customquery cq = ctcBs.saveOrUodateCq(queryId, queryName, querySql, queryDes, loginName,data);
		this.getPageElement("subWinIdBussessId2").setValue(cq.getQueryid());
		//this.closeCueWindowEX();
...
//		String queryId=this.getRadow_parent_data();
	...
	public int setColNames(String code) throws RadowException, AppException{  
		TreeMap<String, String> treeMap = new TreeMap<String, String>();
		if(!"A01".equalsIgnoreCase(code)){
			treeMap = RuleSqlListBS.getVru005byVru004(code,false,"a.isZbx ='1'  ");//仅查询指标项，且忽略vsl006是否配置 mengl 20160629
		}else{
			treeMap = RuleSqlListBS.getVru005byVru004(code,false,"a.isZbx ='1' or  a.columnName = 'AGE' or a.columnName ='A0148' ");//仅查询指标项，且忽略vsl006是否配置 mengl 20160629
		}
