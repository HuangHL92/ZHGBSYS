...
public class gbmcQueryPageModel extends PageModel{
	...
	public int doMemberQuery(int start,int limit) throws RadowException{
		...
		String sql = "select id as uuid,name,sortid, screenshow from A01_GBMC a where a.type = '"+type+"' order by to_number(a.sortid)";
	...
	public int rowdbclick() throws RadowException, AppException{
		...
		this.getExecuteSG().addExecuteCode("if(realParent.groupWin){try {realParent.groupWin.close();} catch (e) {}};window.close();");
		return EventRtnType.NORMAL_SUCCESS;
	}
	...
	@PageEvent("checkBoxClick")
	@AutoNoMask
	public int checkBoxClick(String parm) throws RadowException, SQLException {
		//获取页面数据
		String[] parms = parm.split("@@");
		String uuid = parms[0];
		String screenshow = parms[1];
		HBSession sess = HBUtil.getHBSession();
		sess.createSQLQuery("update a01_gbmc set screenshow=? where id=?")
		.setString(0, "true".equals(screenshow)?"1":"0").setString(1, uuid).executeUpdate();
		sess.flush();
		return EventRtnType.NORMAL_SUCCESS;
	}
}
