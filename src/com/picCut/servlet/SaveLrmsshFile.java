...
public class SaveLrmsshFile implements IAfterFileUpload {
	...
	public Object DoSomethingElse(List<HashMap<String, Object>> list)
		...
		if (list.get(0).get("method") == null||list.get(0).get("method").equals("Again")) {
			for (HashMap<String, Object> map : list) {
				...
				String title_id=map.get("title_id")==null?"":map.get("title_id").toString();
				String publish_id=map.get("publish_id")==null?"":map.get("publish_id").toString();
				...
				if (file.exists()) {
					if (fullpath.endsWith(".lrmx")) {
						...
						personXml = (PersonXml) JXUtil.Xml2Object(xmlStr,
								PersonXml.class);
						obj = saveLrmx(personXml, (String) list.get(0).get("a0201b"),(String)method,title_id,publish_id,map);
					} else {
						if (fullpath.endsWith(".lrm")) {
							String xmlStr = FileUtil.read4String(file);
							obj = saveLrm(xmlStr,
									(String) list.get(0).get("a0201b"), a0000,(String) method,title_id,publish_id);
						...
						if (fullpath.endsWith(".pic")||fullpath.endsWith(".jpg")) {
							...
							String sql = "select a0000 from a01 where a0101 = '"+a0101+"'";
							List newA0000s = HBUtil.getHBSession().createSQLQuery(sql).list();
							if(newA0000s!=null && newA0000s.size()==1){*/
								savePic(file, a0000);
							/*}*/
				...
				if (file.exists()) {
					if (fullpath.endsWith(".lrmx")) {
						...
						personXml = (PersonXml) JXUtil.Xml2Object(xmlString,
								PersonXml.class);
						obj = updateLrmx(personXml, (String) list.get(0).get("a0201b"),title_id,publish_id,map);

					} else {
						if (fullpath.endsWith(".lrm")) {
							 xmlStr = FileUtil.read4String(file);
							obj = updateLrm(xmlStr,
									(String) list.get(0).get("a0201b"),title_id,publish_id);
						}
						if (fullpath.endsWith(".pic")||fullpath.endsWith(".jpg")) {
	...
	public static String updateLrm(String str, String a0201b,String title_id,String publish_id) throws Exception {
...
/*				if(isNumeric(rdsj)){
			...
			//最高学历
			String zgxl="";
			if(a01.getZzxl()==null||"".equals(a01.getZzxl())) {
				zgxl=a01.getQrzxl();
			}else {
				zgxl=a01.getZzxl();
			}
			//最高学位
			String zgxw="";
			if(a01.getZzxw()==null||"".equals(a01.getZzxw())) {
				zgxw=a01.getQrzxw();
			}else {
				zgxw=a01.getZzxw();
			}
			...
			if("".equals(publish_id)||"".equals(title_id)||"null".equals(publish_id)||"null".equals(title_id)) {
				...
				String tp0122="";
				String tp0112="";
				if(a53.getA5304()!=null&&!"".equals(a53.getA5304())) {
					tp0121="1";
				}
				if(a53.getA5315()!=null&&!"".equals(a53.getA5315())) {
					if(a53.getA5315().equals(a01.getA0192a())) {
						tp0122="2";
					}else {
						tp0112=a53.getA5315();
						tp0122="1";
				...
				String zaizhi="在职";
				if(zgxl.equals(a01.getQrzxl())||zgxl.contains("党校")) {
					zaizhi="";
				}
				String shsql="insert into hz_sh_a01 (sh000,a0000,publishid,titleid,a0101,a0104,a0107,a0141,zgxl,zgxw,a0192a,tp0116,tp0117,tp0111,tp0112,tp0113,tp0121,tp0122) "
						+ " values "
						+ "   ('"+UUID.randomUUID().toString()+"','"+a0000+"','"+publish_id+"','"+title_id+"','"+a01.getA0101()+"','"+a01.getA0104()+"'"
						+ "      ,'"+a01.getA0107()+"','"+a01.getA0141()+"','"+zaizhi+zgxl+"','"+zgxw+"','"+a01.getA0192a()+"','1','1'"
						+ "		 ,'"+a53.getA5304()+"','"+tp0112+"','"+a01.getRmly()+"','"+tp0121+"','"+tp0122+"') ";
				HBUtil.executeUpdate(shsql);
			}
			...
			if (value[20].length() > 12) {
				...
				for (int i = 0; i < b.length; i++) {
					...
					if (b[i].length() > 1) {
						a36.setA3604a(codeValue("GB4761", b[i]));
					}
					...
					if (f.length > i && !StringUtil.isEmpty(f[i])) {
						a36.setA3627(codeValue("GB4762", f[i]));
					}
	...
	public static String updateLrmx(PersonXml personXml, String a0201b,String title_id,String publish_id, HashMap<String, Object> map)
			throws Exception {
		HBSession sess = HBUtil.getHBSession();
...
/*				if(isNumeric(rdsj)){
			...
			a01.setQrzxl(personXml.getQuanRiZhiJiaoYu_XueLi()==null?"":personXml.getQuanRiZhiJiaoYu_XueLi().replace("\\n", "").trim());
			a01.setQrzxw(personXml.getQuanRiZhiJiaoYu_XueWei()==null?"":personXml.getQuanRiZhiJiaoYu_XueWei().replace("\\n", "").trim());
			a01.setQrzxlxx(personXml.getQuanRiZhiJiaoYu_XueLi_BiYeYuanXiaoXi()==null?"":personXml.getQuanRiZhiJiaoYu_XueLi_BiYeYuanXiaoXi().replace("\\n", "").trim());
			a01.setQrzxwxx(personXml.getQuanRiZhiJiaoYu_XueWei_BiYeYuanXiaoXi()==null?"":personXml.getQuanRiZhiJiaoYu_XueWei_BiYeYuanXiaoXi().replace("\\n", "").trim());
			a01.setZzxl(personXml.getZaiZhiJiaoYu_XueLi()==null?"":personXml.getZaiZhiJiaoYu_XueLi().replace("\\n", "").trim());
			a01.setZzxw(personXml.getZaiZhiJiaoYu_XueWei()==null?"":personXml.getZaiZhiJiaoYu_XueWei().replace("\\n", "").trim());
			a01.setZzxlxx(personXml.getZaiZhiJiaoYu_XueLi_BiYeYuanXiaoXi()==null?"":personXml.getZaiZhiJiaoYu_XueLi_BiYeYuanXiaoXi().replace("\\n", "").trim());
			a01.setZzxwxx(personXml.getZaiZhiJiaoYu_XueWei_BiYeYuanXiaoXi()==null?"":personXml.getZaiZhiJiaoYu_XueWei_BiYeYuanXiaoXi().replace("\\n", "").trim());
			...
			a01.setA0163("1");
			a01.setA0195(A0195(a0201b));
			a01.setCbdw(personXml.getTianBiaoRen());//填报人
			...
			a53.setA0000(a0000);
			a53.setA5304(personXml.getNiRenZhiWu()==null?"":personXml.getNiRenZhiWu().replace("\\n", "").trim());
			a53.setA5315(personXml.getNiMianZhiWu()==null?"":personXml.getNiMianZhiWu().replace("\\n", "").trim());
			...
			if (personXml.getJiaTingChengYuan() != null
				...
				for (int i = 0; i < personXml.getJiaTingChengYuan().get(0)
					...
					if (ix != null) {
							...
							a36.setA3627(codeValue("GB4762",
									ix.getZhengZhiMianMao()));
							//从map中取出当前称谓对应的序号
							a36.setSortid(sortList.get(i));
						/* a36.setSortid(new BigDecimal(i + 1)); */
							sess.saveOrUpdate(a36);
...
//			throw new Exception(e.getMessage());
	...
	public static String saveLrm(String str, String a0201b, String a0000,String method,String title_id,String publish_id)
			throws Exception {
		HBSession sess = HBUtil.getHBSession();
		HBTransaction trans = null;
		str = str.substring(1, str.length());
...
//								a08.setA0814(qzrxlxx);
//							}
							if(qzrxlxx != null && !"".equals(qzrxlxx.trim())){
								a08.setA0814(qzrxlxx);
							}
							if(qzrxwxx != null && !"".equals(qzrxwxx.trim())){
								a08.setA0824(qzrxwxx);
							}
							a08.setA0899("true");
							a08.setA0837("1");
							sess.save(a08);
...
//								a08.setA0814(zzxlxx);
//							}
							if(zzxlxx != null && !"".equals(zzxlxx.trim())){
								a08.setA0814(zzxlxx);
							}
							if(zzxwxx != null && !"".equals(zzxwxx.trim())){
								a08.setA0824(zzxwxx);
...
//					}
	...
	public static String saveLrmx(PersonXml personXml, String a0201b,String method,String title_id,String publish_id, HashMap<String, Object> map)
			throws Exception {
		HBSession sess = HBUtil.getHBSession();
		HBTransaction trans = sess.beginTransaction();
		try {
